...
class PythonInfo:  # noqa: PLR0904
    ...
    def _init_identity(self) -> None:
        ...
        if self.implementation == "PyPy":
            self.pypy_version_info = tuple(
                sys.pypy_version_info
            )  # ty: ignore[unresolved-attribute] # pypy only
    ...
    def _init_prefixes(self) -> None:
        ...
        try:
            ...
            ImportError
        ):  # pragma: no cover # venv is always available in standard CPython
            has = False
        self.has_venv = has
        self.path = sys.path
    ...
    def _init_schemes(self) -> None:
        ...
        # debian / ubuntu python 3.10 without `python3-distutils` will report mangled `local/bin` / etc. names
            ...
            sys.version_info[:2] == (3, 10) and "deb_system" in scheme_names
        ):  # pragma: no cover # Debian/Ubuntu 3.10
    ...
    def _resolve_tk_lib(tcl: tk.Tk, tcl_lib: str) -> str | None:  # pragma: no cover
        ...
        for version in (
            tk_version,
            ".".join(tk_version.split(".")[:2]),
            tk_version.split(".")[0],
        ):
            tk_lib_path = os.path.join(tcl_parent, f"tk{version}")
            if os.path.isdir(tk_lib_path) and os.path.exists(
                os.path.join(tk_lib_path, "tk.tcl")
            ):
                return tk_lib_path
        return None
    ...
    def _fast_get_system_executable(self) -> str | None:
        ...
        return self._try_posix_fallback_executable(
            base_executable
        )  # pragma: >=3.11 cover

    def _resolve_executable_symlink(
        self, path: str, *, framework: bool | None = None
    ) -> str:
        """
        ...
        if (
            framework
        ):  # macOS framework builds self-locate via dyld from the real binary; e.g. for Homebrew
        ...
        while os.path.islink(result):
            ...
            candidate = (
                ...
                else os.path.normpath(os.path.join(os.path.dirname(result), link))
            )
            # normpath through a symlinked directory may point at a different file - stop resolving there
            if not (
                os.path.exists(candidate) and os.path.samefile(real_path, candidate)
            ):
                return result
            result = candidate
        return result
    ...
    def _stdlib_landmark_exists(dir_path: str) -> bool:
        ...
        return any(
            os.path.exists(os.path.join(dir_path, os.pardir, lib, lib_name, "os.py"))
    ...
    def install_path(self, key: str) -> str:
        ...
        if (
            result is None
        ):  # pragma: >=3.11 cover # distutils is empty when "venv" scheme is available
    ...
    def _distutils_install() -> dict[str, str]:
        ...
        with warnings.catch_warnings():  # disable warning for PEP-632
            ...
            try:
                from distutils import (
                    dist,
                )  # noqa: PLC0415  # ty: ignore[unresolved-import]
        ...
        if hasattr(sys, "_framework"):  # pragma: no cover # macOS framework builds only
            sys._framework = (
                None  # noqa: SLF001  # disable macOS static paths for framework
            )
        ...
        install.prefix = (
            os.sep
        )  # paths generated are relative to prefix that contains the path sep
        install.finalize_options()
        return {
            key: (getattr(install, f"install_{key}")[1:]).lstrip(os.sep)
    ...
    def satisfies(
        self, spec: PythonSpec, *, impl_must_match: bool
    ) -> bool:  # noqa: PLR0911
        """
        ...
        if spec.free_threaded is not None and spec.free_threaded != self.free_threaded:
            return False
        if (
            spec.version_specifier is not None
            and not self._satisfies_version_specifier(spec)
        ):
            return False
        return all(
    ...
    def _satisfies_path(self, spec: PythonSpec) -> bool:
        ...
        if sys.platform == "win32":
            ...
            spec_path = (
                spec_path[: -len(suffix)]
                if suffix and spec_path.endswith(suffix)
                else spec_path
            )
    ...
    def _satisfies_version_specifier(self, spec: PythonSpec) -> bool:
        ...
        for specifier in spec.version_specifier:
            ...
            release = ".".join(
                str(c)
                for c in [version_info.major, version_info.minor, version_info.micro][
                    :precision
                ]
            )
            if (
                ...
                and (
                    precision == 3 or specifier.version.pre_type is not None
                )  # noqa: PLR2004
                and (
    ...
    @classmethod
    def resolve_to_system(
        cls, cache: PyInfoCache | None, target: PythonInfo
    ) -> PythonInfo:
        """
        ...
        for folder in possible_folders:
            for name in possible_names:
                info = self._check_exe(
                    cache, folder, name, discovered, env, exact=exact
                )
                if info is not None:
    ...
    @staticmethod
    def _select_most_likely(
        discovered: list[PythonInfo], target: PythonInfo
    ) -> PythonInfo:
    ...
    def _find_possible_exe_names(self) -> list[str]:
        ...
        for name in self._possible_base():
            for at in (3, 2, 1, 0):
                ...
                for mod, debug, arch, ext in product(
                    mods, debug_suffixes, archs, EXTENSIONS
                ):
