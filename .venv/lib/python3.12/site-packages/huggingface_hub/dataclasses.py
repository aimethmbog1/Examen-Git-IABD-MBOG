...
_TYPED_DICT_DEFAULT_VALUE = (
    object()
)  # used as default value in TypedDict fields (to distinguish from None)
...
def strict(
    cls: Type[T] | None = None, *, accept_kwargs: bool = False
        ...
        if get_origin(value) is Annotated:
            ...
            fields.append(
                (
                    ...
                    base,
                    field(
                        default=_TYPED_DICT_DEFAULT_VALUE,
                        metadata={"validator": meta[0]},
                    ),
                )
            )
        else:
...
def _validate_literal(name: str, value: Any, args: tuple[Any, ...]) -> None:
    ...
    if isinstance(value, bool):
        ...
        if value not in [
            arg for arg in args if isinstance(arg, int) and not isinstance(arg, bool)
        ]:
            raise TypeError(f"Field '{name}' expected one of {args}, got {value}")
...
def _validate_sequence(name: str, value: Any, args: tuple[Any, ...]) -> None:
    ...
    if not isinstance(value, collections.abc.Sequence):
        raise TypeError(
            f"Field '{name}' expected a Sequence, got {type(value).__name__}"
        )
...
def _is_required_or_notrequired(type_hint: Any) -> bool:
    ...
    return type_hint in (Required, NotRequired) or (
        get_origin(type_hint) in (Required, NotRequired)
    )
...
_BASIC_TYPE_VALIDATORS[
    types.UnionType
] = _validate_union  # x | y syntax, available only Python 3.10+
