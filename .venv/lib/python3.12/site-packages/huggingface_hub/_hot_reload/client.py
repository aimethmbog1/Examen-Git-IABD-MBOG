...
def multi_replica_reload_events(
    ...
    MultiReplicaStreamWarning
    | MultiReplicaStreamEvent
    | MultiReplicaStreamReplicaHash
    | MultiReplicaStreamFullMatch
]:
    clients = [
        ReloadClient(
            host=host,
    ...
    for client_index, client in enumerate(clients):
        ...
        while isinstance((events := client.get_reload(commit_sha)), int):
            ...
            if (status_code := events) not in (200, 204):
                raise Exception(
                    f"Unexpected {status_code=} on `ReloadClient.get_reload`"
                )
            subject = "reloadId" if status_code == 204 else "replica"
            yield {
                "kind": "warning",
                "message": f"Retrying on unexpected {subject} not found",
            }
            time.sleep(2)
        ...
        for event_index, event in enumerate(events):
            if client_index == 0:
                ...
                and first_client_events.get(event_index) == event
            ):
                replay.append(event)
                continue
            while replay:
