...
def _open_as_mime_bytes(content: ContentT | None) -> MimeBytes | None:
    ...
    if hasattr(content, "read"):  # duck-typing instead of isinstance(content, BinaryIO)
        ...
        data = content.read()
        mime_type = (
            mimetypes.guess_type(str(content.name))[0]
    ...
    if isinstance(content, Path):
        ...
        return MimeBytes(
            content.read_bytes(), mime_type=mimetypes.guess_type(content)[0]
        )
...
) -> str | TextGenerationStreamOutput | None:
...
def _set_unsupported_text_generation_kwargs(
    model: str | None, unsupported_kwargs: list[str]
) -> None:
