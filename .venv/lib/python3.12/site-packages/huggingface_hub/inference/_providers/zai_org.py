...
class ZaiTextToImageTask(ZaiTask):
    ...
    def _poll_for_result(
        ...
        session = get_session()
        base_url = request_params.url.rsplit(
            "/api/paas/v4/async/images/generations", 1
        )[0]
        ...
        for _ in range(_MAX_POLL_ATTEMPTS):
            ...
            if task_status == "SUCCESS":
                ...
                raise ValueError(
                    f"Zai text-to-image generation failed for request {task_id}"
                )
    ...
    def _extract_image(self, result: dict) -> bytes:
        ...
        if (
            not image_result
            or not isinstance(image_result, list)
            or len(image_result) == 0
        ):
