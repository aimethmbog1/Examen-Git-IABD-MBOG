...
class TogetherTextGenerationTask(BaseTextGenerationTask):
    ...
    def get_response(
        self, response: bytes | dict, request_params: RequestParameters | None = None
    ) -> Any:
...
class TogetherTextToSpeechTask(TogetherTask):
    ...
    def get_response(
        ...
        if isinstance(response, bytes):
            return response
        raise ValueError(
            f"Expected raw audio bytes for text-to-speech, got {type(response).__name__}."
        )
...
class TogetherVideoTask(TogetherTask, ABC):
    ...
    def get_response(
        ...
        if request_params is None:
            raise ValueError(
                "A `RequestParameters` object is required to poll Together video jobs."
            )
        ...
        if status == "failed":
            ...
            raise RuntimeError(
                f"Together video generation failed: {error.get('message') or 'Unknown error'}"
            )
        if status != "completed":
...
class TogetherImageToVideoTask(TogetherVideoTask):
    ...
    def _prepare_payload_as_dict(
        ...
        return {
            ...
            "frame_images": [
                {
                    "input_image": _as_url(inputs, default_mime_type="image/png"),
                    "frame": "first",
                }
            ],
            **_normalize_video_parameters(parameters),
