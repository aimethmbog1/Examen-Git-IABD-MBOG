...
class FalAIImageSegmentationTask(FalAIQueueTask):
    ...
    def get_response(
        ...
        if "image" not in result_dict:
            raise ValueError(
                f"Response from fal ai image-segmentation API does not contain an image: {result_dict}"
            )

        image_data = result_dict["image"]
        if "url" not in image_data:
            raise ValueError(
                f"Image data from fal ai image-segmentation API does not contain a URL: {image_data}"
            )
