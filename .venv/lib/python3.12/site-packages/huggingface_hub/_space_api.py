...
class SpaceHotReloading:
    ...
    replica_statuses: list[
        tuple[str, str | None]
    ]  # See _hot_reloading_types.ApiCreateReloadResponse.res.status
    raw: dict
...
class SpaceRuntime:
    ...
    def __init__(self, data: dict) -> None:
        ...
        self.hot_reloading = (
            SpaceHotReloading(raw_hr)
            if (raw_hr := data.get("hotReloading")) is not None
            else None
        )
        raw_volumes = data.get("volumes")
        self.volumes = (
            [Volume(**v) for v in raw_volumes] if raw_volumes is not None else None
        )
        self.raw = data
