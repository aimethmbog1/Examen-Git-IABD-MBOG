...
class _BucketAddFile:
    ...
    def __post_init__(self) -> None:
        ...
        self.mtime = int(
            os.path.getmtime(self.source) * 1000
    ...
    def __init__(self, **kwargs):
        ...
        self.uploaded_at = (
            (
                ...
                else parse_datetime(uploaded_at)
            )
            if uploaded_at
            else None
        )
...
def _parse_bucket_uri(path: str) -> HfUri:
    ...
    if path.startswith(constants.HF_PROTOCOL) or _looks_like_hf_url(path):
        ...
        if not parsed.is_bucket:
            raise ValueError(
                f"Invalid bucket path: {path}. Must be a bucket URI (hf://buckets/...)."
            )
        return parsed
    parts = path.split("/", 2)
    if len(parts) < 2 or not parts[0] or not parts[1]:
        raise ValueError(
            f"Invalid bucket path: '{path}'. Expected format: namespace/bucket_name"
        )
...
class SyncOperation:
    ...
    bucket_file: BucketFile | None = (
        None  # BucketFile when available (not serialized to plan file)
    )
...
class SyncPlan:
    ...
    def summary(self) -> dict[str, int | str]:
        ...
        total_size = sum(
            ...
            if op.action in ("upload", "download")
        )
        return {
            "uploads": uploads,
            "downloads": downloads,
    ...
    *,
    ...
    if ignore_sizes:
        ...
        if size_differs or source_newer:
            ...
            return SyncOperation(
                action=action, reason=reason, bucket_file=bucket_file, **base_kwargs
            )
        else:
...
def _compute_sync_plan(
    ...
    if not is_upload and not is_download:
        raise ValueError(
            "One of source or dest must be a bucket path (hf://buckets/...) and the other must be local."
        )
    ...
    if is_upload:
        ...
        if status:
            try:
                ...
                pass
        try:
            for rel_path, size, mtime_ms, _ in _list_remote_files(
                api, bucket_id, prefix
            ):
                if filter_matcher.matches(rel_path):
                ...
                if status:
                    ...
                    status.update(
                        f"Scanning remote bucket ({len(remote_files)}{total_str} files)"
                    )
        ...
        if status:
            try:
                ...
                pass
        for rel_path, size, mtime_ms, bucket_file in _list_remote_files(
            api, bucket_id, prefix
        ):
            if filter_matcher.matches(rel_path):
                remote_files[rel_path] = (size, mtime_ms)
        ...
        if os.path.isdir(local_path):
            if delete:
                ...
                for rel_path, size, mtime_ms in _list_local_files(local_path):
                    ...
                    if status:
                        status.update(
                            f"Scanning local directory ({len(local_files)} files)"
                        )
            else:
...
def _execute_plan(
    plan: SyncPlan, api: "HfApi", verbose: bool = False, status: Any | None = None
) -> None:
    """Execute a sync plan."""
...
def sync_bucket_internal(
    ...
    *,
    ...
    if source_is_bucket and dest_is_bucket:
        raise ValueError(
            "Remote to remote sync is not supported. One path must be local."
        )

    if not source_is_bucket and not dest_is_bucket:
        raise ValueError(
            "One of source or dest must be a bucket path (hf://buckets/...)."
        )
