...
def _http_backoff_base(
    ...
    *,
    ...
    retry_on_exceptions: type[Exception]
    | tuple[type[Exception], ...] = _DEFAULT_RETRY_ON_EXCEPTIONS,
    ...
    **kwargs,
    ...
    ratelimit_reset: int | None = (
        None  # seconds to wait for rate limit reset if 429 response
    )
    ...
    while True:
        ...
        if ratelimit_reset is not None:
            ...
            logger.warning(
                f"Retrying in {actual_sleep}s [Retry {nb_tries}/{max_retries}]."
            )
...
def hf_raise_for_status(
    response: httpx.Response, endpoint_name: str | None = None
) -> None:
    """
    ...
    try:
        ...
        repo_type, repo_id = (
            _parse_repo_info_from_url(request_url) if request_url else (None, None)
        )
        ...
        if error_code == "RevisionNotFound":
            ...
            raise _format(
                GatedRepoError, message, response, repo_type=repo_type, repo_id=repo_id
            ) from e
            ...
            raise _format(
                ...
                message,
                response,
                bucket_id=_parse_bucket_id_from_url(request_url),
            ) from e
            ...
            if ratelimit_info is not None:
                ...
                if (
                    ratelimit_info.limit is not None
                    and ratelimit_info.window_seconds is not None
                ):
                    message += (
...
def _warn_on_warning_headers(response: httpx.Response) -> None:
    ...
    for server_warning in server_warnings:
        topic, message = (
            server_warning.split(";", 1)
            if ";" in server_warning
            else ("", server_warning)
        )
        topic = topic.strip()
