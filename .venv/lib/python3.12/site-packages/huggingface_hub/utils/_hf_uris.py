...
_TYPE_TO_PREFIX: dict[str, str] = {
    v: k for k, v in constants.HF_URI_TYPE_PREFIXES.items()
}
...
_URL_REPO_LOCATION_ACTIONS: frozenset[str] = frozenset(
    {"blame", "blob", "raw", "resolve", "tree"}
)
...
class HfUri:
    ...
    def __post_init__(self) -> None:
        ...
        if self.type not in _VALID_URI_TYPES:
            raise HfUriError(
                uri=uri,
                msg=f"Invalid type '{self.type}'. Must be one of {sorted(_VALID_URI_TYPES)}.",
            )

        # Check valid ID
        if not self.id or self.id.count("/") != 1:
            raise HfUriError(
                uri=uri, msg=f"Id must be 'namespace/name', got '{self.id}'."
            )
        if self.type != "bucket":
            try:
        ...
        if self.path_in_repo:
            if self.path_in_repo.startswith("/") or "//" in self.path_in_repo:
                raise HfUriError(
                    uri=uri,
                    msg=f"Path must not contain empty segments (got '{self.path_in_repo}').",
                )
    ...
    def to_uri(self) -> str:
        ...
        """
        parts: list[str] = [
            constants.HF_PROTOCOL,
            _TYPE_TO_PREFIX[self.type],
            "/",
            self.id,
        ]
        if self.revision is not None:
            ...
            if (
                "/" in revision
                and _SPECIAL_REFS_REVISION_REGEX.fullmatch(revision) is None
            ):
    ...
    def to_url(self, endpoint: str | None = None) -> str:
        ...
        url = (
            f"{base}/{self.id}"
            if self.type == "model"
            else f"{base}/{_TYPE_TO_PREFIX[self.type]}/{self.id}"
        )
        revision = self.revision
...
def _looks_like_hf_url(uri: str) -> bool:
    ...
    return any(
        ...
        for host in constants.HF_URL_HOSTS
    )
...
def _url_to_uri_body(url: str) -> str:
    ...
    if not segments:
        raise HfUriError(
            uri=raw, msg=f"Missing repository or bucket identifier in URL '{url}'."
        )
    ...
    if type_prefix == "buckets":
        ...
        if action not in _URL_BUCKET_LOCATION_ACTIONS:
            raise HfUriError(
                uri=raw,
                msg=f"Cannot parse bucket URL '{url}': unsupported '/{action}/' route.",
            )
...
def parse_hf_mount(mount_str: str) -> HfMount:
    ...
    if mount_path is None:
        raise HfUriError(
            uri=raw,
            msg="Missing mount path. Expected ':<MOUNT_PATH>' (e.g. 'hf://org/model:/data').",
        )
...
def _split_type(location: str, *, raw: str) -> tuple[constants.HfUriType, str]:
    ...
    if (singular_plural := _TYPE_TO_PREFIX.get(first)) is not None:
        raise HfUriError(
            uri=raw,
            msg=f"Type prefix must be plural, got '{first}/'. Did you mean '{singular_plural}/'?",
        )
    return "model", location
...
def _parse_bucket_body(
    ...
    *,
    ...
    if len(parts) < 2 or not parts[0] or not parts[1]:
        raise HfUriError(
            uri=raw, msg=f"Bucket id must be 'namespace/name', got '{location}'."
        )
    bucket_id = f"{parts[0]}/{parts[1]}"
    if "@" in bucket_id:
        raise HfUriError(
            uri=raw, msg="Bucket URIs do not support a revision marker ('@')."
        )
    ...
    *,
    ...
    if at_idx == -1 or location[:at_idx].count("/") > 1:
        ...
        if len(parts) < 2:
            raise HfUriError(
                uri=raw,
                msg=f"Repository id must be 'namespace/name', got '{location}'. ",
            )
        ...
        if repo_id.count("/") != 1:
            raise HfUriError(
                uri=raw, msg=f"Repository id must be 'namespace/name', got '{repo_id}'."
            )
