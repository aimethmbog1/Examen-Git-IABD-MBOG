...
def parse_xet_file_data_from_response(
    response: httpx.Response, endpoint: str | None = None
) -> XetFileData | None:
    """
...
def fetch_xet_connection_info_from_repo_info(
    *,
    ...
    return _fetch_xet_connection_info_with_url(
        url, headers, params, cache_key_prefix=f"{repo_type}-{repo_id}"
    )
...
def reset_xet_connection_info_cache_for_repo(
    repo_type: str | None, repo_id: str
) -> None:
...
def _cache_key(
    ...
    }  # casing is not guaranteed here
    auth_header = lower_headers.get("authorization", "")
    params_str = "&".join(
        f"{k}={v}" for k, v in sorted((params or {}).items(), key=lambda x: x[0])
    )
...
def _is_expired(connection_info: XetConnectionInfo) -> bool:
    ...
    return (
        ...
        <= int(time.time()) + XET_CONNECTION_INFO_SAFETY_PERIOD
    )
