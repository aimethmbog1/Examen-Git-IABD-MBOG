...
class XetProgressReporter:
    ...
    def update_progress(self, group_report, item_reports: dict):
        ...
        transfer_inc = (
            group_report.total_transfer_bytes_completed
            - self._prev_transfer_bytes_completed
        )
        self._prev_transfer_bytes_completed = (
            group_report.total_transfer_bytes_completed
        )
        ...
        total_files_count = (
            self.total_files if self.total_files is not None else len(self.known_items)
        )
        self.data_processing_bar.set_description(
            self.format_desc(
                f"Processing Files ({len(self.completed_items)} / {total_files_count})",
                False,
            ),
            refresh=False,
        )
        self.data_processing_bar.set_postfix_str(
            postfix(group_report.total_bytes_completion_rate), refresh=False
        )
        ...
        self.upload_bar.set_postfix_str(
            postfix(group_report.total_transfer_bytes_completion_rate), refresh=False
        )
