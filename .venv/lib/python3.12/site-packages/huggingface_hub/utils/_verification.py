...
def _resolve_commit_hash_from_cache(storage_folder: Path, revision: str | None) -> str:
    ...
    if revision:
        ...
        raise ValueError(
            f"Revision '{revision}' could not be resolved in cache (expected file '{ref_path}')."
        )
    ...
    if not snapshots_dir.is_dir():
        raise ValueError(
            f"Cache repo is missing snapshots directory: {snapshots_dir}. Provide --revision explicitly."
        )

    candidates = [
        p.name
        for p in snapshots_dir.iterdir()
        if p.is_dir() and _REGEX_COMMIT_HASH.fullmatch(p.name)
    ]
    if len(candidates) == 1:
        return candidates[0]
    ...
    *,
    ...
    for rel_path in both:
        ...
        if actual != expected:
            mismatches.append(
                Mismatch(
                    path=rel_path, expected=expected, actual=actual, algorithm=algorithm
                )
            )
...
def resolve_local_root(
    *,
    ...
    if local_dir is not None:
        ...
        if not root.is_dir():
            raise ValueError(
                f"Local directory does not exist or is not a directory: {root}"
            )
    ...
    if not snapshot_dir.is_dir():
        raise ValueError(
            f"Snapshot directory does not exist for revision '{commit}': {snapshot_dir}."
        )
