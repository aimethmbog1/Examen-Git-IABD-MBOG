...
class _Cached(_cached_base):
    ...
    def __call__(cls, *args, **kwargs):
        ...
        fs_token_main_thread = cls._tokenize(
            cls, threading.main_thread().ident, *args, **kwargs
        )
...
class HfFileSystem(
    fsspec.AbstractFileSystem, metaclass=_Cached
    ...
    * Buckets (generic storage):
    ...
    def __init__(
        ...
        **storage_options,
        ...
        self._repo_and_revision_exists_cache: dict[
            tuple[str, str, str | None], tuple[bool, Exception | None]
        ] = {}
        # Same for buckets
    ...
    def resolve_path(
        ...
        if not path:
            raise NotImplementedError(
                "Access to buckets and repositories lists is not implemented."
            )
        if path.count("/") == 0:
            raise ValueError(
        ...
        if parsed.is_bucket:
            ...
            return HfFileSystemResolvedBucketPath(
                bucket_id=parsed.id, path=parsed.path_in_repo
            )
        ...
        if (
            ...
            path_without_type = (
                path.split("/", 1)[1]
                if path.split("/")[0] in constants.HF_URI_TYPE_PREFIXES
                else path
            )
            repo_id, after_at = path_without_type.split("@", 1)
            revision_in_path, path_in_repo = (
                after_at.split("/", 1) if "/" in after_at else (after_at, "")
            )
            ...
            return HfFileSystemResolvedRepositoryPath(
                ...
                revision,
                path_in_repo,
                _raw_revision=revision_in_path,
            )
        ...
        repo_and_revision_exist, err = self._repo_and_revision_exist(
            parsed.type, parsed.id, revision
        )
        if not repo_and_revision_exist:
            _raise_file_not_found(path, err)
        ...
        if "@" in path and parsed.revision is not None:
            ...
            raw_revision = (
                raw_after_at[: -(len(parsed.path_in_repo) + 1)]
        ...
        **kwargs,
        ...
        if block_size == 0:
            return HfFileSystemStreamFile(
                self, path, mode=mode, revision=revision, **kwargs
            )
        else:
    ...
    def _rm(self, path: str, revision: str | None = None, **kwargs) -> None:
        ...
        if isinstance(resolved_path, HfFileSystemResolvedBucketPath):
            self._api.batch_bucket_files(
                resolved_path.bucket_id, delete=[resolved_path.path]
            )
        else:
    ...
    def rm(
        ...
        **kwargs,
        ...
        if isinstance(resolved_path, HfFileSystemResolvedBucketPath):
            delete = [
                self.resolve_path(path).path for path in paths if not self.isdir(path)
            ]
    ...
    def _ls_tree(
        ...
        expand_info = (
            ...
            else (self.expand_info if self.expand_info is not None else False)
        )
        resolved_path = self.resolve_path(path, revision=revision)
        path = resolved_path.unresolve()
        ...
        if path in self.dircache and not refresh:
            ...
            dirs_not_expanded = []
            if expand_info and isinstance(
                resolved_path, HfFileSystemResolvedRepositoryPath
            ):
    ...
    def _list_bucket_tree_with_folders(
        ...
        bucket_files = self._api.list_bucket_tree(
            bucket_id, prefix, recursive=recursive
        )
        ...
        for bucket_entry in bucket_files:
            ...
            for parent_bucket_folder_str in list(
                PurePosixPath(bucket_entry.path).parents
            )[: -min_depth - 1]:
                parent_bucket_folder = BucketFolder(
                    type="directory",
                    path=str(parent_bucket_folder_str),
                    uploaded_at=bucket_entry.uploaded_at,
                )
        ...
        if not out:
            raise EntryNotFoundError(
                f"File not found in bucket '{bucket_id}': '{prefix}'"
            )
        return out
    ...
    def find(
        ...
        **kwargs,
        ...
        try:
            out = self._ls_tree(
                path, recursive=True, refresh=refresh, maxdepth=maxdepth, **kwargs
            )
        except EntryNotFoundError:
            # Path could be a file
            try:
    ...
    def cp_file(
        self, path1: str, path2: str, revision: str | None = None, **kwargs
    ) -> None:
        """
    ...
    def modified(self, path: str, **kwargs) -> datetime:
        ...
        if "last_commit" in info:
            if info["last_commit"] is None:
                raise NotImplementedError(
                    f"'modified' is not implemented for repository paths like '{path}'"
                )
            ...
            raise NotImplementedError(
                f"Cannot determined 'modified' for path '{path}' (info: {info})"
            )

    def info(
        self, path: str, refresh: bool = False, revision: str | None = None, **kwargs
    ) -> dict[str, Any]:
        """
    ...
    def _get_instance_state(self):
        return {
            ...
            "_repo_and_revision_exists_cache": deepcopy(
...
class HfFileSystemFile(fsspec.spec.AbstractBufferedFile):
    def __init__(
        self, fs: HfFileSystem, path: str, revision: str | None = None, **kwargs
    ):
        try:
    ...
    def _fetch_range(self, start: int, end: int) -> bytes:
        ...
        url = self.url()
        r = http_backoff(
            "GET", url, headers=headers, timeout=constants.HF_HUB_DOWNLOAD_TIMEOUT
        )
        ...
        if final:
            ...
            if isinstance(self.resolved_path, HfFileSystemResolvedBucketPath):
                self.fs._api.batch_bucket_files(
                    self.resolved_path.bucket_id,
                    add=[(self.temp_file.name, self.resolved_path.path)],
                )
            else:
