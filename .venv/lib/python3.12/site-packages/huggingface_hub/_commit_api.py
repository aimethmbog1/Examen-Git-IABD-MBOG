    ...
    *,
    ...
    for chunk in chunk_iterable(additions, chunk_size=UPLOAD_BATCH_MAX_NUM_FILES):
        ...
        has_buffered_io_data = any(
            isinstance(op.path_or_fileobj, io.BufferedIOBase) for op in chunk_list
        )
        if is_xet_available():
...
def _upload_xet_files(
    ...
    **How it works:**
    ...
    try:
        ...
        all_bytes_ops = [
            op for op in additions if isinstance(op.path_or_fileobj, bytes)
        ]
        all_paths_ops = [
            op for op in additions if isinstance(op.path_or_fileobj, (str, Path))
        ]
        ...
        with session.new_upload_commit(
            ...
            for op in all_paths_ops:
                commit.start_upload_file(
                    str(op.path_or_fileobj), sha256=op.upload_info.sha256.hex()
                )
            for op in all_bytes_ops:
                commit.start_upload_bytes(
                    op.path_or_fileobj, sha256=op.upload_info.sha256.hex()
                )
    ...
    for (src_repo_id_key, src_repo_type_key, src_revision_key), group in groupby(
        ...
        for paths_batch in chunk_iterable(src_paths, FETCH_LFS_BATCH_SIZE):
            ...
            for src_repo_file in src_repo_files:
                ...
                if not is_cross_repo:
                    oid_info[
                        (src_repo_file.path, src_revision_key)
                    ] = src_repo_file.blob_id
        ...
        for operation in operations:
            ...
            if key not in files_to_copy:
                source_desc = (
                    f" from {eff_repo_type}s/{eff_repo_id}" if is_cross_repo else ""
                )
