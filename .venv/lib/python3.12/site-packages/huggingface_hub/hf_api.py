...
CollectionItemType_T = Literal[
    "model", "dataset", "space", "paper", "collection", "bucket"
]
...
def _resolve_repo_visibility(
    *,
    ...
    if private is not None and visibility is not None:
        raise ValueError(
            "Received both `private` and `visibility` arguments. Please provide only one of them."
        )
    ...
    if visibility == "protected" and repo_type != constants.REPO_TYPE_SPACE:
        raise ValueError(
            "Only Spaces can be 'protected'. Please set visibility to 'public' or 'private'."
        )
    return visibility


def repo_type_and_id_from_hf_id(
    hf_id: str, hub_url: str | None = None
...
class CommitInfo(str):
    ...
    def __post_init__(self):
        ...
        self.repo_url = RepoUrl(
            self.commit_url.split("/commit/")[0], endpoint=self._endpoint
        )
...
def _resolve_copy_target_path(
    ...
    if src_root_path is None:
        ...
        raise ValueError(
            f"Unexpected source path while copying folder: '{src_file_path}'."
        )
    ...
    if (
        ...
        and src_root_path is not None
        and not merge_contents
    ):
...
class ModelInfo:
    ...
    def __init__(self, **kwargs):
        ...
        self.eval_results = (
            parse_eval_result_entries(eval_results) if eval_results else None
        )
...
class PaperAuthor:
    ...
    def __init__(self, **kwargs) -> None:
        ...
        self.status_last_changed_at = (
            parse_datetime(status_last_changed_at) if status_last_changed_at else None
        )
...
class PaperInfo:
    ...
    def __init__(self, **kwargs) -> None:
        ...
        self.linked_models = (
            [ModelInfo(**m) for m in linked_models]
        ...
        self.linked_datasets = (
            [DatasetInfo(**d) for d in linked_datasets]
        ...
        self.linked_spaces = (
            [SpaceInfo(**s) for s in linked_spaces]
            if linked_spaces is not None
            else None
        )
...
def _get_safetensors_metadata_size(
    size_bytes: bytes, filename: str, context_msg: str
) -> int:
    """
...
def _parse_safetensors_header(
    metadata_as_bytes: bytes, filename: str, context_msg: str
) -> SafetensorsFileMetadata:
    """
...
class HfApi:
    ...
    def whoami(self, token: bool | str | None = None, *, cache: bool = False) -> dict:
        ...
        if token is False:
            raise ValueError(
                "Cannot use `token=False` with `whoami` method as it requires authentication."
            )
        ...
        try:
            ...
            if e.response.status_code == 401:
                ...
                if token == _get_token_from_google_colab():
                    ...
                    error_message += " The token stored is invalid. Please run `hf auth login --force` to set a new token."
    ...
    @_deprecate_arguments(
        version="2.0",
        deprecated_args=["model_name"],
        custom_message="Use `search` instead.",
    )
    @validate_hf_hub_args
    def list_models(
        self,
        *,
        ...
        inference_provider: Literal["all"]
        | PROVIDER_T
        | list[PROVIDER_T]
        | None = None,
        ...
        if trained_dataset:
            datasets = (
                ...
                else trained_dataset
            )
            filter_list.extend(
                f"dataset:{d}" if not d.startswith("dataset:") else d for d in datasets
            )
    ...
    def list_dataset_parquet_files(
        ...
        *,
        ...
        response = get_session().get(
            url, params=params, headers=self._build_hf_headers(token=token)
        )
    ...
    def list_user_repos(
        ...
        *,
        ...
        if namespace is not None:
            path = (
                f"{self.endpoint}/api/organizations/{namespace}/settings/repositories"
            )
        else:
        ...
        *,
        ...
        path = (
            ...
            if revision is None
                ...
                f"{self.endpoint}/api/kernels/{repo_id}/revision/{quote(revision, safe='')}"
            )
        )
        r = get_session().get(path, headers=headers, timeout=timeout)
        hf_raise_for_status(r)
    ...
    def create_repo(
        ...
        *,
        ...
        resolved_visibility = _resolve_repo_visibility(
            private=private, visibility=visibility, repo_type=repo_type
        )
        ...
        space_args: list[tuple[str, str, Any]] = [
            ...
            (
                "space_volumes",
                "volumes",
                [v.to_dict() for v in space_volumes] if space_volumes else None,
            ),
        ]
        ...
        if repo_type == constants.REPO_TYPE_SPACE:
            for _, key, value in space_args:
                if value is not None:
                    payload[key] = value
            if (
                space_sleep_time is not None
                and space_hardware == SpaceHardware.CPU_BASIC
            ):
                warnings.warn(
            ...
            if provided_space_args := [
                arg for arg, _, value in space_args if value is not None
            ]:
                warnings.warn(
        ...
        try:
            ...
            if exist_ok and err.response.status_code == 409:
                ...
                try:
                    ...
                    return RepoUrl(
                        f"{self.endpoint}/{constants.REPO_TYPES_URL_PREFIXES[repo_type]}{repo_id}"
                    )
        ...
        *,
        ...
        try:
            commit_resp = get_session().post(
                url=commit_url, headers=headers, content=data, params=params
            )
    ...
    def _duplicate_lfs_files(
        ...
        *,
        ...
        cross_repo_copies = [
            op for op in copies if op.src_repo_id is not None and not op._is_duplicated
        ]
        ...
        for (src_repo_id, src_repo_type, src_revision), group in itertools.groupby(
            cross_repo_copies,
            key=lambda op: (op.src_repo_id, op.src_repo_type, op.src_revision),
        ):
    ...
    def parse_safetensors_file_metadata(
        ...
        *,
        ...
        context_msg = (
            f"repo '{repo_id}', revision '{revision or constants.DEFAULT_REVISION}'"
        )
        ...
        metadata_size = _get_safetensors_metadata_size(
            response.content[:8], filename, context_msg
        )
    ...
    def get_space_secrets(
        self, repo_id: str, *, token: bool | str | None = None
    ...
    def list_spaces_hardware(
        ...
        """
        response = get_session().get(
            f"{self.endpoint}/api/spaces/hardware",
            headers=self._build_hf_headers(token=token),
        )
        hf_raise_for_status(response)
    ...
    def set_space_sleep_time(
        self, repo_id: str, sleep_time: int, *, token: bool | str | None = None
    ) -> SpaceRuntime:
        ...
        *,
        ...
        while True:
            if error_to_retry is not None:
                logger.warning(
                    f"'{error_to_retry}' thrown while requesting {log_label}"
                )
                logger.warning(
                    f"Retrying in {sleep_time}s [Retry {nb_tries}/{max_retries}]."
                )
                error_to_retry = None
            ...
            try:
                ...
                is_no_new_line_timeout = isinstance(
                    err, (httpx.ReadTimeout, httpcore.ReadTimeout)
                )
                ...
                if on_iteration_end is not None:
                    ...
                    if is_no_new_line_timeout or type(err) in tolerated_exception_types:
                        ...
                        nb_tries += 1
                        sleep_time = min(
                            max_wait_time, max(min_wait_time, sleep_time * 2)
                        )
                        error_to_retry = err
                else:
    ...
    def update_inference_endpoint(
        ...
        *,
        ...
        if scaling_metric:
            payload["compute"]["scaling"]["measure"] = {
                scaling_metric: scaling_threshold
            }
    ...
    def get_collection(
        self, collection_slug: str, *, token: bool | str | None = None
    ) -> Collection:
    ...
    def list_pending_access_requests(
        ...
        *,
        ...
        ```
        """
        yield from self._list_access_requests(
    ...
    @validate_hf_hub_args
    def get_webhook(
        self, webhook_id: str, *, token: bool | str | None = None
    ) -> WebhookInfo:
    ...
    def get_organization_overview(
        self, organization: str, token: bool | str | None = None
    ) -> Organization:
        """
    ...
    def list_user_followers(
        self, username: str, token: bool | str | None = None
    ) -> Iterable[User]:
        """
    ...
    def list_daily_papers(
        ...
        *,
        ...
        r = get_session().get(
            path, params=params, headers=self._build_hf_headers(token=token)
        )
        hf_raise_for_status(r)
        for paper in r.json():
            yield PaperInfo(**paper)
    ...
    def _fetch_running_job_sse(
        ...
        *,
        ...
        def has_job_finished() -> bool:
            ...
            return "status" in job_status and job_status["status"]["stage"] not in (
                "RUNNING",
                "UPDATING",
            )
    ...
    def list_jobs_hardware(
        ...
        """
        response = get_session().get(
            f"{self.endpoint}/api/jobs/hardware",
            headers=self._build_hf_headers(token=token),
        )
        hf_raise_for_status(response)
        return [JobHardwareInfo(**hardware) for hardware in response.json()]
    ...
    def list_scheduled_jobs(
        ...
        *,
        ...
        hf_raise_for_status(response)
        return [
            ScheduledJobInfo(**scheduled_job_info)
            for scheduled_job_info in response.json()
        ]
    ...
    def _create_uv_command_env_and_secrets(
        ...
        *,
        ...
        for local_file_to_include in local_files_to_include:
            ...
            if remote_file_path.name in remote_to_local_file_names:
                for i in itertools.count():
                    remote_file_name = remote_file_path.with_stem(
                        remote_file_path.stem + f"({i})"
                    ).name
                    if remote_file_name not in remote_to_local_file_names:
                        remote_to_local_file_names[
                            remote_file_name
                        ] = local_file_to_include
                        break
            else:
                remote_to_local_file_names[
        ...
        script_args = [
            f"{mount_path}/{local_to_remote_file_names[arg]}"
    ...
    def _upload_scripts_to_bucket(
        ...
        *,
        ...
        subfolder_id = (
            f"{datetime.now(timezone.utc).strftime('%Y%m%dT%H%M%S')}-{token_hex(3)}"
        )

        bucket_url = self.create_bucket(
            bucket_id=bucket_id, exist_ok=True, token=token, private=True
        )
        ...
        print(
            f"Your script and Job artifacts will be saved in this bucket: {bucket_url.url}"
        )
        ...
        *,
        ...
        if "/" not in bucket_id:
            namespace, name = (
                "me",
                bucket_id,
            )  # "me" namespace refers to the current user
        else:
        ...
        try:
            ...
            if exist_ok and err.response.status_code == 409:
                ...
                try:
                    ...
                    return BucketUrl(
                        f"{self.endpoint}/buckets/{bucket_id}", endpoint=self.endpoint
                    )
                except HfHubHTTPError:
                    raise err
            else:
    ...
    def list_buckets(
        ...
        *,
        ...
        if search is not None:
            params["search"] = search
        for item in paginate(
            f"{self.endpoint}/api/buckets/{namespace}",
            params=params,
            headers=self._build_hf_headers(token=token),
        ):
    ...
    @validate_hf_hub_args
    def copy_files(
        self, source: str, destination: str, *, token: str | bool | None = None
    ) -> None:
    ...
    def _copy_to_bucket(
        ...
        *,
        ...
        if destination_path == "":
            ...
            dest_path_info = list(
                self.get_bucket_paths_info(
                    destination_bucket_id, [destination_path], token=token
                )
            )
            if dest_path_info:
                destination_is_directory = False
                ...
                destination_is_directory = (
                    destination_exists_as_directory or destination_str.endswith("/")
                )
        ...
        def _resolve_target_path(
            src_file_path: str, src_root_path: str | None, is_single_file: bool
        ) -> str:
        ...
        def _add_repo_file(file: RepoFile, target_path: str) -> None:
            if file.xet_hash is not None:
                all_copies.append(
                    _build_copy_op(
                        target_path, file.xet_hash, file.size, source.type, source.id
                    )
                )
            else:
        ...
        if source.is_bucket:
            ...
            source_path_info = list(
                self.get_bucket_paths_info(source.id, [source_path], token=token)
            )

            if source_path_info:
                source_file = source_path_info[0]
                target_path = _resolve_target_path(
                    source_file.path, None, is_single_file=True
                )
                all_copies.append(
                    _build_copy_op(
                        target_path,
                        source_file.xet_hash,
                        source_file.size,
                        "bucket",
                        source.id,
                    )
                ...
                for item in self.list_bucket_tree(
                    source.id, prefix=source_path or None, recursive=True, token=token
                ):
                    if not isinstance(item, BucketFile):
                        continue
                    if source_path and not (
                        item.path == source_path
                        or item.path.startswith(source_path + "/")
                    ):
                        continue
                    target_path = _resolve_target_path(
                        item.path, source_path or None, is_single_file=False
                    )
                    all_copies.append(
                        _build_copy_op(
                            target_path, item.xet_hash, item.size, "bucket", source.id
                        )
                    )
        else:
        ...
        if not all_copies and not all_adds and not pending_downloads:
            if source.is_bucket:
                raise EntryNotFoundError(
                    f"No files found at '{source_str}' in bucket '{source.id}'."
                )
            else:
                raise EntryNotFoundError(
                    f"No files found at '{source_str}' in {source.type} '{source.id}'."
                )
        ...
        if all_copies:
            for copy_chunk in chunk_iterable(
                all_copies, chunk_size=_BUCKET_BATCH_ADD_CHUNK_SIZE
            ):
                self._batch_bucket_files(
                    destination_bucket_id, copy=list(copy_chunk), token=token
                )
        if all_adds:
            for add_chunk in chunk_iterable(
                all_adds, chunk_size=_BUCKET_BATCH_ADD_CHUNK_SIZE
            ):
                self._batch_bucket_files(
                    destination_bucket_id, add=list(add_chunk), token=token
                )
    ...
    def _iter_repo_files_for_copy(
        ...
        *,
        ...
        if len(source_repo_path_info) == 1 and isinstance(
            source_repo_path_info[0], RepoFile
        ):
            ...
            for repo_item in self.list_repo_tree(
                ...
                if not isinstance(repo_item, RepoFile):
                    continue
                yield repo_item, _resolve(
                    repo_item.path, source_path or None, is_single_file=False
                )
    ...
    def batch_bucket_files(
        ...
        *,
        ...
        try:
            for copy_chunk in chunk_iterable(
                copy, chunk_size=_BUCKET_BATCH_ADD_CHUNK_SIZE
            ):
                self._batch_bucket_files(bucket_id, copy=list(copy_chunk), token=token)

            for add_chunk in chunk_iterable(
                add, chunk_size=_BUCKET_BATCH_ADD_CHUNK_SIZE
            ):
                self._batch_bucket_files(
                    bucket_id, add=list(add_chunk), token=token, _progress=progress
                )

            for delete_chunk in chunk_iterable(
                delete, chunk_size=_BUCKET_BATCH_DELETE_CHUNK_SIZE
            ):
                self._batch_bucket_files(
                    bucket_id, delete=list(delete_chunk), token=token
                )
        finally:
    ...
    def _batch_bucket_files(
        ...
        *,
        ...
        if add:
            for add_item in add:
                if isinstance(add_item, _BucketAddFile):
                    ...
                    operations.append(
                        _BucketAddFile(source=source, destination=destination)
                    )
        if copy:
        ...
        add_bytes_operations = [
            op for op in add_operations if isinstance(op.source, bytes)
        ]
        add_path_operations = [
            op for op in add_operations if not isinstance(op.source, bytes)
        ]
                ...
                with session.new_upload_commit(
                            ...
                            handles.append(
                                (
                                    commit.start_upload_file(
                                        str(op.source), sha256=SKIP_SHA256
                                    ),
                                    op,
                                )
                            )
                    for op in add_bytes_operations:
                        if op.xet_hash is None:
                            handles.append(
                                (
                                    commit.start_upload_bytes(
                                        op.source, sha256=SKIP_SHA256
                                    ),
                                    op,
                                )
                            )
    ...
    def get_bucket_file_metadata(
        ...
        *,
        ...
        if xet_file_data is None:
            raise ValueError(
                f"Could not parse xet file data for '{remote_path}' in bucket '{bucket_id}'."
            )

        size = response.headers.get("Content-Length")
        if size is None:
            raise ValueError(
                f"Could not get size for '{remote_path}' in bucket '{bucket_id}'."
            )
        ...
        *,
        ...
        if str_paths:
            bucket_files_by_path = {
                info.path: info
                for info in self.get_bucket_paths_info(
                    bucket_id, str_paths, token=token
                )
            }

            # Check for missing files
            missing_paths = [
                path for path in str_paths if path not in bucket_files_by_path
            ]
            if missing_paths:
                ...
                for path in missing_paths:
                    warnings.warn(
                        f"File '{path}' not found in bucket '{bucket_id}'. Skipping."
                    )
        ...
        for remote_file, local_path in files:
            ...
            if remote_file.size == 0:
                ...
                if dest_path.exists():
                    if dest_path.is_dir():
                        raise IsADirectoryError(
                            f"Expected file but found directory at '{dest_path}'"
                        )
                ...
                non_zero_download_items.append(
                    (
                        XetFileInfo(remote_file.xet_hash, remote_file.size),
                        str(dest_path),
                    )
                )
...
def parse_local_safetensors_file_metadata(path: str | Path) -> SafetensorsFileMetadata:
    ...
    with open(path, "rb") as f:
        ...
        size_bytes = f.read(8)
        metadata_size = _get_safetensors_metadata_size(
            size_bytes, filename, context_msg
        )
...
def get_local_safetensors_metadata(path: str | Path) -> SafetensorsRepoMetadata:
    ...
    if path.is_file():
        ...
        return SafetensorsRepoMetadata(
            ...
            sharded=False,
            weight_map={
                tensor_name: path.name for tensor_name in file_metadata.tensors.keys()
            },
    ...
    if index_file_path.exists():
        ...
        for shard_filename in set(weight_map.values()):
            ...
            files_metadata[shard_filename] = parse_local_safetensors_file_metadata(
                shard_path
            )
