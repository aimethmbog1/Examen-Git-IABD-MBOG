...
def datasets_ls(
    repo_id: Annotated[
        str | None,
        typer.Argument(
            help="Dataset ID (e.g. `username/repo-name`) to list files from. If omitted, lists datasets."
        ),
    ] = None,
...
def datasets_leaderboard(
    dataset_id: Annotated[
        str,
        typer.Argument(
            help="The benchmark dataset ID (e.g. `SWE-bench/SWE-bench_Verified`)."
        ),
    ],
    limit: LimitOpt = 20,
    token: TokenOpt = None,
    ...
    out.hint(
        "Use 'hf datasets ls --filter benchmark:official' to list available leaderboards."
    )
    if leaderboard:
        out.hint(
            f"Use 'hf models info {leaderboard[0].model_id}' to get details about a model."
        )
...
def datasets_info(
    dataset_id: Annotated[
        str, typer.Argument(help="The dataset ID (e.g. `username/repo-name`).")
    ],
    revision: RevisionOpt = None,
    expand: ExpandOpt = None,
    token: TokenOpt = None,
...
def datasets_parquet(
    ...
    subset: Annotated[
        str | None,
        typer.Option("--subset", help="Filter parquet entries by subset/config."),
    ] = None,
    split: Annotated[
        str | None, typer.Option(help="Filter parquet entries by split.")
    ] = None,
    token: TokenOpt = None,
) -> None:
...
def datasets_card(
    ...
    if metadata:
        ...
        out.hint(
            f"Use `hf datasets card {dataset_id} --metadata` to extract only the card metadata."
        )
