...
def _parse_namespace_from_job_id(
    job_id: str, namespace: str | None
) -> tuple[str, str | None]:
    ...
    if job_id.count("/") > 1:
        raise CLIError(
            f"Job ID must be in the form 'job_id' or 'namespace/job_id': '{job_id}'."
        )
...
STATS_UPDATE_MIN_INTERVAL = (
    0.1  # we set a limit here since there is one update per second per job
)
...
def jobs_run(
    ...
    if isinstance(job.status.expose_urls, list):
        ...
        out.hint(
            f"Exposed ports are reachable at (requires an HF token with read access to the job):\n{urls}"
        )
    if detach:
        job_ref = f"{job.owner.name}/{job.id}"
        out.hint(
            f"Use `hf jobs logs -f {job_ref}` to stream logs, or `hf jobs inspect {job_ref}` to check status."
        )
        return
...
def jobs_logs(
    ...
    api = get_hf_api(token=token)
    try:
        logs = api.fetch_job_logs(
            job_id=job_id, namespace=namespace, follow=follow, tail=tail
        )
        for log in logs:
            out.text(log)
        if follow:
        ...
        if status == 404:
            ...
            raise CLIError(
                "Access denied. You may not have permission to view this job."
            ) from e
        else:
...
def _matches_filters(
    job_properties: dict[str, str], filters: list[tuple[str, str, str]]
) -> bool:
...
def jobs_stats(
    ...
    if job_ids is None:
        job_ids = [
            ...
            if (job.status.stage if job.status else "UNKNOWN")
            in ("RUNNING", "UPDATING")
        ]
    if len(job_ids) == 0:
    ...
    try:
        with multiprocessing.pool.ThreadPool(len(job_ids)) as pool:
            ...
            for job_id in job_ids:
                ...
                row += [
                    "-- / --" if ("/" in header or "USAGE" in header) else "--"
                    for header in table_headers[1:]
                ]
                rows_per_job_id[job_id] = [row]
            last_update_time = time.time()
            total_rows = [
                row for job_id in rows_per_job_id for row in rows_per_job_id[job_id]
            ]
            ...
            kwargs_list = [
                {
                    "job_id": job_id,
                    "metrics_stream": api.fetch_job_metrics(
            ...
            for done, job_id, rows in iflatmap_unordered(
                pool, _get_jobs_stats_rows, kwargs_list=kwargs_list
            ):
                if done:
                ...
                if now - last_update_time >= STATS_UPDATE_MIN_INTERVAL:
                    ...
                    total_rows = [
                        row
                        for job_id in rows_per_job_id
                        for row in rows_per_job_id[job_id]
                    ]
                    print(_tabulate(total_rows, headers=table_headers))
...
def jobs_ps(
    ...
    for f in filter or []:
        if f.startswith("label!=") or f.startswith("label="):
            if f.startswith("label!="):
                ...
                if "=" in label_part:
                    out.warning(
                        f"Ignoring invalid label filter format 'label!={label_part}'. Use label!=key format."
                    )
                    continue
    ...
    if not items:
        if filters:
            ...
            out.hint(
                "No running jobs. Use `-a`/`--all` to include finished (and failed) jobs."
            )
...
def jobs_hardware() -> None:
    ...
    out.table(items)
    out.hint(
        "Use `hf jobs run --flavor <name> ...` to request a specific hardware flavor."
    )
...
def jobs_inspect(
    ...
    try:
        jobs = [
            api.inspect_job(job_id=job_id, namespace=namespace) for job_id in job_ids
        ]
    except HfHubHTTPError as e:
        status = e.response.status_code if e.response is not None else None
        if status == 404:
...
def jobs_cancel(
    ...
    try:
        ...
        if status == 404:
            ...
            raise CLIError(
                "Access denied. You may not have permission to cancel this job."
            ) from e
        else:
...
def jobs_labels(
    ...
    label: LabelsOpt = None,
    clear: Annotated[
        bool, typer.Option("--clear", help="Remove all labels from the job.")
    ] = False,
    namespace: NamespaceOpt = None,
    token: TokenOpt = None,
) -> None:
    ...
    if not label and not clear:
        raise CLIError(
            "Please set at least one label with --label. To remove all labels, pass --clear."
        )
    if label and clear:
        raise CLIError(
...
def scheduled_ps(
    ...
    for sj in filtered_jobs:
        ...
        item["command"] = " ".join(cmd) if cmd else "N/A"
        item["last_run"] = (
            last_job["at"][:19].replace("T", " ")
            if last_job and last_job.get("at")
            else "N/A"
        )
        item["next_run"] = (
            status_dict["next_job_run_at"][:19].replace("T", " ")
            if status_dict.get("next_job_run_at")
            else "N/A"
        )
...
def scheduled_delete(
    ...
    """Delete a scheduled Job."""
    scheduled_job_id, namespace = _parse_namespace_from_job_id(
...
def scheduled_labels(
    ...
    label: LabelsOpt = None,
    clear: Annotated[
        bool, typer.Option("--clear", help="Remove all labels from the scheduled job.")
    ] = False,
    namespace: NamespaceOpt = None,
    token: TokenOpt = None,
) -> None:
...
def _write_generator_to_queue(
    queue: Queue[T], func: Callable[..., Iterable[T]], kwargs: dict
) -> None:
