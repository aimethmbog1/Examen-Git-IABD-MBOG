...
def get_hf_api(token: str | None = None) -> "HfApi":
    ...
    return HfApi(
        token=token, library_name="huggingface-cli", library_version=__version__
    )
...
def _format_epilog_no_indent(
    epilog: str | None, ctx: click.Context, formatter: click.HelpFormatter
) -> None:
...
class HFCliTyperGroup(TyperGroup):
            ...
            if e.ctx is not None and e.ctx.command is not None:
                ...
                if isinstance(cmd, click.Group):
                    ...
                    items = [
                        ...
                        if (sub := cmd.get_command(e.ctx, name)) is not None
                        and not sub.hidden
                    ]
                    ...
                    items = [
                        ...
                        and not p.hidden
                        and (record := p.get_help_record(e.ctx))
                    ]
        ...
        try:
            ...
            if cmd is None and cmd_name is not None:
                ...
                items = [
                    ...
                    if (sub := self.get_command(ctx, name)) is not None
                    and not sub.hidden
                ]
                _enrich_usage_error(e, "commands", items)
            raise
    ...
    def _rewrite_repo_type_prefix(cmd: click.Command, args: list[str]) -> None:
        ...
        *prefix* is one of ``spaces``, ``datasets``, or ``models``), the prefix is
        ...
        """
        has_type_option = any(
            isinstance(param, click.Option) and "--type" in param.opts
        ...
        if any(
            arg == "--type"
            or arg.startswith("--type=")
            or arg == "--repo-type"
            or arg.startswith("--repo-type=")
            for arg in args
        ):
            raise click.UsageError(
...
_FORMATTING_OPTIONS_HELP_RECORDS: list[tuple[str, str]] = [
    ...
    (
        "--no-truncate",
        "Do not truncate scalar values in human tables (list/dict columns stay shortened).",
    ),
]
...
def _consume_format_flags_for_leaf(cmd: click.Command, args: list[str]) -> None:
    ...
    * **Modern commands** (no local format/quiet/json options): the flags '--format <value>' / '--json' / '--quiet' / '-q' are stripped from 'args' and applied to the singleton 'out'.
    ...
    if has_local_format:
        _rewrite_legacy_shorthands(
            args, rewrite_json=not has_local_json, rewrite_quiet=not has_local_quiet
        )
        return
    ...
    def _check_conflict(new_flag: str) -> None:
        ...
        if chosen_flag is not None:
            raise click.UsageError(
                f"'{chosen_flag}' and '{new_flag}' are mutually exclusive."
            )
...
def _rewrite_legacy_shorthands(
    args: list[str], *, rewrite_json: bool, rewrite_quiet: bool
) -> None:
    ...
    """
    has_format_in_args = any(
        arg == "--format" or arg.startswith("--format=") for arg in args
    )
    ...
    if rewrite_quiet:
        ...
        if flag is not None:
            if has_format_in_args:
                raise click.UsageError(
                    f"'{flag}' and '--format' are mutually exclusive."
                )
            idx = args.index(flag)
...
def _parse_format_value(value: str) -> "OutputFormat":
    try:
        ...
        raise click.UsageError(
            f"Invalid value for '--format': '{value}'. Valid values: {valid}."
        ) from None


def _enrich_usage_error(
    error: click.UsageError, label: str, items: list[tuple[str, str]]
) -> None:
...
def HFCliCommand(
    topic: TOPIC_T, examples: list[str] | None = None
) -> type[TyperCommand]:
    def format_epilog(
    ...
    def format_options(
        self: TyperCommand, ctx: click.Context, formatter: click.HelpFormatter
    ) -> None:
    ...
    def parse_args(
        self: click.Command, ctx: click.Context, args: list[str]
    ) -> list[str]:
...
def typer_factory(
    help: str, epilog: str | None = None, cls: type[TyperGroup] | None = None
) -> "HFCliApp":
...
FilterOpt = Annotated[
    list[str] | None,
    typer.Option(
        help="Filter by tags (e.g. 'text-classification'). Can be used multiple times."
    ),
]
...
def parse_env_map(
    ...
    if env_file:
        env_map.update(
            load_dotenv(Path(env_file).read_text(), environ=extended_environ)
        )
...
def _fetch_latest_brew_version() -> str | None:
    ...
    try:
        response = get_session().get(
            "https://formulae.brew.sh/api/formula/hf.json", timeout=2
        )
        hf_raise_for_status(response)
        return response.json()["versions"]["stable"]
    except Exception:
        logger.debug(
            "Error while fetching latest version from Homebrew.", exc_info=True
        )
        return None
