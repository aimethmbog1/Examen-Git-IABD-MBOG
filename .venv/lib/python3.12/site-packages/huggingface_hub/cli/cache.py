...
_FILTER_PATTERN = re.compile(
    r"^(?P<key>[a-zA-Z_]+)\s*(?P<op>==|!=|>=|<=|>|<|=)\s*(?P<value>.+)$"
)
_ALLOWED_OPERATORS = {"=", "!=", ">", "<", ">=", "<="}
...
def print_cache_selected_revisions(
    ...
    for repo in sorted(
        selected_by_repo.keys(), key=lambda repo: (repo.repo_type, repo.repo_id.lower())
    ):
        ...
        for revision in revisions:
            ...
            out.text(
                f"      {revision.commit_hash} [{refs}] {revision.size_on_disk_str}"
            )
...
def _repo_cache_id_from_target(target: str) -> str:
    ...
    if uri.revision is not None or uri.path_in_repo:
        raise CLIError(
            "Only repo-level hf:// URIs are supported by `hf cache rm` for now."
        )
...
def collect_cache_entries(
    ...
    sorted_repos = sorted(
        hf_cache_info.repos, key=lambda repo: (repo.repo_type, repo.repo_id.lower())
    )
    for repo in sorted_repos:
        repo_refs_map[repo] = frozenset(
            {ref for revision in repo.revisions for ref in revision.refs}
        )
...
def compile_cache_filter(
    ...
    if op not in _ALLOWED_OPERATORS:
        raise ValueError(
            f"Unsupported operator '{op}' in filter '{expr}'. Must be one of {list(_ALLOWED_OPERATORS)}."
        )

    if key not in _FILTER_KEYS:
        raise ValueError(
            f"Unsupported filter key '{key}' in '{expr}'. Must be one of {list(_FILTER_KEYS)}."
        )
    ...
    if key in {"modified", "accessed"}:
        ...
        def _time_filter(
            repo: CachedRepoInfo, revision: CachedRevisionInfo | None, now: float
        ) -> bool:
            timestamp = (
                repo.last_accessed
                if key == "accessed"
    ...
    if key == "type":
        ...
        def _type_filter(
            repo: CachedRepoInfo, revision: CachedRevisionInfo | None, _: float
        ) -> bool:
        ...
        def _refs_filter(
            ...
            refs = (
                revision.refs
                if revision is not None
                else repo_refs_map.get(repo, frozenset())
            )
...
def compile_cache_sort(
    ...
    if not match:
        raise ValueError(
            f"Invalid sort expression: '{sort_expr}'. Expected format: 'key' or 'key:asc' or 'key:desc'."
        )
    ...
    if key not in _SORT_KEYS:
        raise ValueError(
            f"Unsupported sort key '{key}' in '{sort_expr}'. Must be one of {list(_SORT_KEYS)}."
        )
...
def _resolve_deletion_targets(
    hf_cache_info: HFCacheInfo, targets: list[str]
) -> _DeletionResolution:
...
def ls(
    ...
    entries, repo_refs_map = collect_cache_entries(
        hf_cache_info, include_revisions=revisions
    )
    try:
...
def rm(
    ...
    out.text(
        f"About to delete {summary_text} totalling {strategy.expected_freed_size_str}."
    )
...
def prune(
    ...
    for repo in hf_cache_info.repos:
        detached = frozenset(
            revision for revision in repo.revisions if len(revision.refs) == 0
        )
        if not detached:
            continue
...
def verify(
    ...
    if result.mismatches:
        ...
        out.text(
            f"❌ Checksum verification failed for the following file(s):\n{details}"
        )
        exit_code = 1
