...
def upload(
    ...
    if repo_id.startswith(constants.HF_PROTOCOL):
        if repo_type is not None:
            raise CLIError(
                f"'--repo-type' cannot be used with an 'hf://' URI ('{repo_id}')."
            )
        if revision is not None:
            raise CLIError(
                f"'--revision' cannot be used with an 'hf://' URI ('{repo_id}')."
            )
        uri = parse_hf_uri(repo_id)
        if uri.is_bucket:
            raise CLIError(
                "Buckets are not supported by `hf upload`. Use `hf sync` instead."
            )
    ...
    def run_upload() -> str:
        if os.path.isfile(resolved_local_path):
            if (
                resolved_include is not None
                and len(resolved_include) > 0
                and isinstance(resolved_include, list)
            ):
        ...
        if every is not None:
            ...
            out.text(
                f"Scheduling commits every {every} minutes to {scheduler.repo_id}."
            )
            try:
                while True:
                    time.sleep(100)
        ...
        if not os.path.isfile(resolved_local_path) and not os.path.isdir(
            resolved_local_path
        ):
            raise FileNotFoundError(
                f"No such file or directory: '{resolved_local_path}'."
            )
        ...
        if revision is not None and not create_pr:
            try:
                api.repo_info(
                    repo_id=created, repo_type=repo_type_str, revision=revision
                )
    ...
    *,
    ...
    if local_path is not None and any(c in local_path for c in ["*", "?", "["]):
        if include is not None:
            raise ValueError(
                "Cannot set --include when local_path contains a wildcard."
            )
        if path_in_repo is not None and path_in_repo != ".":
            raise ValueError(
                "Cannot set path_in_repo when local_path contains a wildcard."
            )
