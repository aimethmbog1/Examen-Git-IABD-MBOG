...
def make_cp(context: CpContext | None = None):
    ...
    def cp(
        src: Annotated[
            str,
            typer.Argument(
                help="Source: local file, hf:// URI (repo or bucket), or - for stdin."
            ),
        ],
        dst: Annotated[
            str | None,
            typer.Argument(
                help="Destination: local path, hf:// URI (repo or bucket), or - for stdout."
            ),
        ] = None,
        token: TokenOpt = None,
    ) -> None:
...
def _enforce_context(context: CpContext | None, src: str, dst: str | None) -> None:
    ...
    if context == "repos" and parse_hf_uri(remote).is_bucket:
        raise CLIError(
            "`hf repos cp` only works with repositories. Use `hf cp` or `hf buckets cp` for buckets."
        )
    if context == "buckets" and not parse_hf_uri(remote).is_bucket:
        raise CLIError(
            "`hf buckets cp` only works with buckets. Use `hf cp` or `hf repos cp` for repositories."
        )
    ...
    if not src_is_hf and not dst_is_hf:
        if dst is None:
            raise typer.BadParameter(
                "Missing destination. Provide a repo or bucket hf:// URI as DST."
            )
        raise typer.BadParameter(
...
def _download_single(api: HfApi, uri: HfUri, local_path: str) -> None:
    ...
    if uri.is_bucket:
        api.download_bucket_files(
            uri.id, [(uri.path_in_repo, local_path)], raise_on_missing_files=True
        )
    else:
...
def _upload_file_to_remote(
    api: HfApi, src: str, dst: str, *, src_is_stdin: bool
) -> None:
    ...
    if src_is_stdin:
        if uri.path_in_repo == "" or dst.endswith("/"):
            raise typer.BadParameter(
                "Stdin upload requires a full destination path including filename."
            )
...
def _upload_single(
    api: HfApi, uri: HfUri, source: str | bytes, remote_path: str
) -> None:
