        ...
        *,
        ...
        if headers is None:
            ...
            headers = [
                ...
                if any(item.get(col) is not None for item in items)
            ]
        ...
        match self.mode:
            case OutputFormat.human:  # padded table, adaptive truncation, SCREAMING_SNAKE headers
                ...
                formatted_rows: list[list[str]] = [
                    [_format_table_value_human(v) for v in row] for row in rows
                ]

                is_truncated = _truncate_columns(
                    screaming_headers, formatted_rows, no_truncate=self.no_truncate
                )
                ...
                if is_truncated:
                    self.hint(
                        "Use `--no-truncate` or `--format json` to display full values."
                    )
        ...
        *,
        ...
        if self.mode != OutputFormat.human:
            raise ConfirmationError(
                f"{message} Use {confirm_param} to skip confirmation."
            )
...
def _infer_alignments(headers: list[str], rows: list[list[Any]]) -> dict[str, str]:
    ...
    for c, h in enumerate(headers):
        if all(
            row[c] is None
            or (isinstance(row[c], (int, float)) and not isinstance(row[c], bool))
            for row in rows
        ):
            result[h] = "right"
    return result
...
def _truncate_columns(
    ...
    *,
    ...
    natural = [
        max(len(headers[c]), *(len(rows[r][c]) for r in range(len(rows))))
        for c in range(n)
    ]
