...
def models_ls(
    repo_id: Annotated[
        str | None,
        typer.Argument(
            help="Model ID (e.g. `username/repo-name`) to list files from. If omitted, lists models."
        ),
    ] = None,
...
def models_info(
    model_id: Annotated[
        str, typer.Argument(help="The model ID (e.g. `username/repo-name`).")
    ],
    revision: RevisionOpt = None,
    expand: ExpandOpt = None,
    token: TokenOpt = None,
...
def models_card(
    ...
    if metadata:
        ...
        out.hint(
            f"Use `hf models card {model_id} --metadata` to extract only the card metadata."
        )
