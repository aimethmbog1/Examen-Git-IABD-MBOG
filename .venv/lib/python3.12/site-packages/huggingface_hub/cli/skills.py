...
def _format_params(cmd: Command) -> str:
    ...
    for p in cmd.params:
        ...
        if p.name and p.name.startswith("_"):
            continue
        long_name = next(
            (o for o in getattr(p, "opts", []) if o.startswith("--")), None
        )
...
def _collect_leaf_commands(
    group: Group, ctx: Context, path_parts: list[str]
) -> list[tuple[list[str], Command]]:
...
def _get_flag_names(cmd: Command, *, exclude: set[str] | None = None) -> list[str]:
    ...
    if _accepts_global_format_flags(cmd):
        flags.extend(
            flag
            for flag in _GLOBAL_FORMAT_INLINE_FLAGS
            if not (exclude and flag.split()[0] in exclude)
        )
    return flags
...
def _compute_common_flags(
    ...
    for _path, cmd in leaf_commands:
        for p, long_name, short_name in _iter_optional_params(cmd):
            ...
            if long_name not in flag_info or (
                short_name and " / " not in flag_info[long_name][0]
            ):
...
def build_skill_md() -> str:
    ...
    lines.append("")
    lines.append(
        f"Generated with `huggingface_hub v{__version__}`. Run `hf skills add --force` to regenerate."
    )
    lines.append("")
...
def _create_symlink(
    agent_skills_dir: Path, skill_name: str, central_skill_path: Path, force: bool
) -> Path:
    ...
    results = [
        {
            ...
            **{
                label: "yes"
                if label in installed.get(skill.name.lower(), set())
                else ""
...
def skills_update(
    name: Annotated[
        str | None,
        typer.Argument(
            help="Optional installed skill name to update.", show_default=False
        ),
    ] = None,
    claude: Annotated[
        bool, typer.Option("--claude", help="Update skills installed for Claude.")
    ] = False,
    global_: Annotated[
        bool,
        typer.Option(
