...
class ExtensionManifest:
    ...
    def load(cls, path: Path) -> "ExtensionManifest":
        ...
        if not manifest_path.is_file():
            raise CLIError(
                f"Manifest file not found at {manifest_path}. Your extension may be corrupted."
            )
...
def extension_install(
    ...
    repo_id: Annotated[
        str,
        typer.Argument(
            help="GitHub extension repository in `[OWNER/]hf-<name>` format."
        ),
    ],
    force: Annotated[
        bool, typer.Option("--force", help="Overwrite if already installed.")
    ] = False,
) -> None:
    ...
    if extension_exists and not force:
        raise CLIError(
            f"Extension '{short_name}' is already installed. Use --force to overwrite."
        )
...
def extension_exec(
    ...
    exit_code = _execute_extension_binary(
        executable_path=executable_path, args=list(ctx.args)
    )
...
def extension_search() -> None:
    ...
    response = get_session().get(
        ...
        params={
            "q": f"topic:{_EXTENSIONS_GITHUB_TOPIC}",
            "sort": "stars",
            "order": "desc",
            "per_page": 100,
        },
...
def _list_installed_extensions() -> list[ExtensionManifest]:
    ...
    for extension_dir in sorted(root_dir.iterdir()):
        ...
        try:
            ...
            logger.debug(
                f"Failed to load manifest for extension '{extension_dir.name}': {e}"
            )
            continue
    return manifests
...
def list_installed_extensions_for_help() -> list[tuple[str, str]]:
    ...
    for manifest in _list_installed_extensions():
        ...
        help_text = (
            f"{manifest.description} {tag}" if manifest.description is not None else tag
        )
        entries.append((manifest.short_name, help_text))
    return entries


def dispatch_unknown_top_level_extension(
    args: list[str], known_commands: set[str]
) -> int | None:
    if not args:
        return None
    ...
    return _execute_extension_binary(
        executable_path=executable_path, args=list(args[1:])
    )
...
def _auto_install_official_extension(short_name: str) -> Path | None:
    ...
    try:
        out.confirm(
            f"'{short_name}' is an official Hugging Face extension ({owner}/{repo_name}). Install it?"
        )
...
def _install_extension_from_github(
    *,
    ...
    try:
        binary = _fetch_remote_binary(
            owner=owner, repo_name=repo_name, branch=branch, short_name=short_name
        )
    except Exception:
        binary = None
...
def _fetch_remote_binary(
    owner: str, repo_name: str, branch: str, short_name: str
) -> bytes:
    ...
    response = get_session().get(
        raw_url, follow_redirects=True, timeout=_EXTENSIONS_DOWNLOAD_TIMEOUT
    )
    response.raise_for_status()
...
def _install_python_extension(
    *, owner: str, repo_name: str, short_name: str, extension_dir: Path, branch: str
    ...
    source_url = (
        f"https://github.com/{owner}/{repo_name}/archive/refs/heads/{branch}.zip"
    )
    ...
    try:
        ...
        raise CLIExtensionInstallError(
            f"Failed to set up pip extension from '{owner}/{repo_name}': {e}"
        ) from e
    finally:
        if not installed:
            shutil.rmtree(extension_dir, ignore_errors=True)
...
def _normalize_repo_id(repo_id: str) -> tuple[str, str, str]:
    if "://" in repo_id:
        raise CLIError(
            "Only GitHub repositories in `[OWNER/]hf-<name>` format are supported."
        )
    ...
    if not repo_name.startswith("hf-"):
        raise CLIError(
            f"Extension repository name must start with 'hf-', got '{repo_name}'."
        )
