...
variables_cli = typer_factory(
    help="Manage environment variables for a Space on the Hub."
)
...
def spaces_ls(
    repo_id: Annotated[
        str | None,
        typer.Argument(
            help="Space ID (e.g. `username/repo-name`) to list files from. If omitted, lists spaces."
        ),
    ] = None,
    ...
    human_readable: Annotated[
        ...
        typer.Option(
            "--human-readable",
            "-h",
            help="Show sizes in human readable format (only for listing files).",
        ),
    ] = False,
    as_tree: Annotated[
        bool,
        typer.Option(
            "--tree", help="List files in tree format (only for listing files)."
        ),
    ] = False,
    recursive: Annotated[
        bool,
        typer.Option(
            "--recursive", "-R", help="List files recursively (only for listing files)."
        ),
    ] = False,
...
def spaces_info(
    space_id: Annotated[
        str, typer.Argument(help="The space ID (e.g. `username/repo-name`).")
    ],
    revision: RevisionOpt = None,
    expand: ExpandOpt = None,
    token: TokenOpt = None,
...
def spaces_card(
    ...
    metadata: Annotated[
        bool, typer.Option("--metadata", help="Output only the metadata from the card.")
    ] = False,
    text: Annotated[
        bool, typer.Option("--text", help="Output only the text body (no metadata).")
    ] = False,
    token: TokenOpt = None,
) -> None:
    ...
    if metadata:
        ...
        out.hint(
            f"Use `hf spaces card {space_id} --metadata` to extract only the card metadata."
        )
...
def spaces_search(
    ...
    sdk: Annotated[
        list[str] | None,
        typer.Option(help="Filter by SDK (e.g. gradio, docker, static)."),
    ] = None,
    include_non_running: Annotated[
        bool, typer.Option(help="Include non-running spaces in results.")
    ] = False,
    description: Annotated[
        bool, typer.Option(help="Show AI-generated descriptions.")
    ] = False,
    limit: LimitOpt = 10,
    token: TokenOpt = None,
) -> None:
    ...
    for r in itertools.islice(results, limit):
        item: dict = {
            ...
            "score": round(r.semantic_relevancy_score, 2)
            if r.semantic_relevancy_score is not None
            else None,
        }
        if description:
...
def dev_mode(
    ...
    folder = getattr(
        info.card_data,
        "dev-mode-folder",
        "" if info.sdk == "docker" else "/home/user/app",
    )
    ...
    print("")
    print(
        "PS: Dev mode stops after 48h of inactivity, don't forget to save your changes regularly."
    )
...
def spaces_ssh(
    ...
    auto: Annotated[
        bool,
        typer.Option(
            "--auto", help="Enable Dev Mode without prompting if not already enabled."
        ),
    ] = False,
    token: TokenOpt = None,
) -> None:
    ...
    if info.runtime is None or not info.runtime.dev_mode:
        out.confirm(
            f"Dev Mode is disabled on '{space_id}'. Enable it now?",
            yes=auto,
            default=True,
            confirm_param="--auto",
        )
...
def spaces_hardware(token: TokenOpt = None) -> None:
    ...
    for hw in hardware_list:
        accelerator = (
            f"{hw.accelerator.quantity}x {hw.accelerator.model} ({hw.accelerator.vram})"
            if hw.accelerator
            else None
        )
    ...
    out.table(items)
    out.hint(
        "Use `hf spaces settings <space_id> --hardware <name>` to request hardware for a Space."
    )
...
def spaces_logs(
    ...
    if not found_logs and not build:
        out.hint(
            f"No run logs found for space {space_id}. Try passing --build to fetch build logs instead."
        )
...
def spaces_hot_reload(
    ...
    skip_checks: Annotated[
        bool, typer.Option(help="Skip hot-reload compatibility checks.")
    ] = False,
    skip_summary: Annotated[
        bool, typer.Option(help="Skip summary display after hot-reload is triggered")
    ] = False,
    token: TokenOpt = None,
) -> None:
    """
    ...
    if not skip_checks:
        ...
        if space_info.sdk != "gradio":
            raise CLIError(
                f"Hot-reloading is only available on Gradio SDK. Found {space_info.sdk} SDK"
            )
        ...
        if version.parse(sdk_version) < version.Version(HOT_RELOADING_MIN_GRADIO):
            raise CLIError(
                f"Hot-reloading requires Gradio >= {HOT_RELOADING_MIN_GRADIO} (found {sdk_version})"
            )
    ...
    if local_file:
        ...
        with disable_progress_bars():
            try:
                ...
                typer.secho(
                    f"{filename} not found in remote repository. Assuming new file",
                    fg=typer.colors.BRIGHT_BLACK,
                )
        ...
        if editor_res == "no-tty":
            ...
            typer.secho(
                f"- Run `hf spaces hot-reload {space_id} {filename} -f {local_path}`"
            )
            return
        if editor_res == "no-editor":
            raise CLIError(
                "No editor found in local environment. Use -f flag to hot-reload from local path"
            )
        if editor_res != 0:
            raise CLIError(
                f"Editor returned a non-zero exit code while attempting to edit {local_path}"
            )
    else:
    ...
    if local_file is not None and local_file.resolve().is_relative_to(Path.cwd()):
        ...
        typer.secho(
            "Consider running `git pull --autostash` to stay synced if you are working from a local clone."
        )
...
def _spaces_hot_reload_summary(
    ...
    while (space_info := api.space_info(space_id)).sha == current_sha:
        if current_sha is None or current_sha == commit_sha:
            break
        typer.secho(
            "Waiting for up-to-date Space infos", fg=typer.colors.BRIGHT_BLACK, err=True
        )
        time.sleep(2)
    if space_info.sha != commit_sha:
        raise CLIError(
            f"Expected SHA {commit_sha} after hot-reload but got {space_info.sha}"
        )
    ...
    if (hot_reloading := runtime.hot_reloading) is None:
        raise CLIError(
            f"Space {space_id} current running version has not been hot-reloaded"
        )
    if hot_reloading.status != "created":
        typer.echo(
            f"Failed creating hot-reloaded commit. {hot_reloading.replica_statuses=}"
        )
        return
    ...
    def display_event(event: ApiGetReloadEventSourceData) -> None:
        if event["data"]["kind"] == "error":
            ...
            typer.secho(
                f"✘ Exception at {render_region(event['data']['region'])}", bold=True
            )
            typer.secho(event["data"]["traceback"], italic=True)
        elif event["data"]["kind"] == "add":
            typer.secho(
                f"✔︎ Created {event['data']['objectName']} {event['data']['objectType']}",
                bold=True,
            )
        elif event["data"]["kind"] == "delete":
            typer.secho(
                f"∅ Deleted {event['data']['objectName']} {event['data']['objectType']}",
                bold=True,
            )
        elif event["data"]["kind"] == "update":
            typer.secho(
                f"✔︎ Updated {event['data']['objectName']} {event['data']['objectType']}",
                bold=True,
            )
    ...
    for replica_stream_event in multi_replica_reload_events(
        ...
        if replica_stream_event["kind"] == "event":
            ...
            typer.secho(
                f"⚠ {replica_stream_event['message']}", fg=typer.colors.BRIGHT_BLACK
            )
        else:
...
def volumes_delete(
    ...
    out.confirm(
        f"You are about to remove all volumes from Space '{space_id}'. Proceed?",
        yes=yes,
    )
    api = get_hf_api(token=token)
...
def secrets_ls(
    ...
    out.table(items)
    out.hint(
        f"Use `hf spaces secrets add {space_id} -s KEY=VALUE` to add secrets to a Space."
    )
...
def secrets_add(
    ...
    if not secrets_map:
        raise CLIError(
            "At least one secret must be specified with -s/--secrets or --secrets-file."
        )
    api = get_hf_api(token=token)
    ...
    out.hint(
        f"Use `hf spaces secrets delete {space_id} <key>` to remove a secret from a Space."
    )
...
def secrets_delete(
    ...
    out.hint(
        f"Use `hf spaces secrets add {space_id} -s {key}=<value>` to re-add a secret to a Space."
    )
...
def variables_ls(
    ...
    out.table(items)
    out.hint(
        f"Use `hf spaces variables add {space_id} -e KEY=VALUE` to add variables to a Space."
    )
...
def variables_add(
    ...
    if not env_map:
        raise CLIError(
            "At least one variable must be specified with -e/--env or --env-file."
        )
    api = get_hf_api(token=token)
...
def variables_delete(
    ...
    out.hint(
        f"Use `hf spaces variables ls {space_id}` to list remaining variables for a Space."
    )
