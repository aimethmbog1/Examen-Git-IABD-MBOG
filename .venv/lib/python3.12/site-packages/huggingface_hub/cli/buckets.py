    ...
    items = [
        {
            ...
            "size": format_size(bucket.size, human_readable)
            if human_readable
            else bucket.size,
...
def _list_files(
    ...
    print_file_listing(
        items, human_readable=human_readable, as_tree=as_tree, recursive=recursive
    )
...
def remove(
    ...
    if recursive:
        ...
        if dry_run:
            out.text(
                "\n".join(
                    f"delete: {BUCKET_PREFIX}{bucket_id}/{path}" for path in file_paths
                )
            )
