...
@ie_cli.command(
    "list | ls", examples=["hf endpoints ls", "hf endpoints ls --namespace my-org"]
)
def ls(
...
catalog_app.command(name="list | ls", examples=["hf endpoints catalog ls"])(
    list_catalog
)
...
def describe(
    ...
    try:
        endpoint = api.get_inference_endpoint(
            name=name, namespace=namespace, token=token
        )
    except HfHubHTTPError as error:
...
def pause(
    ...
    api = get_hf_api(token=token)
    try:
        endpoint = api.pause_inference_endpoint(
            name=name, namespace=namespace, token=token
        )
    except HfHubHTTPError as error:
...
def scale_to_zero(
    ...
    api = get_hf_api(token=token)
    try:
        endpoint = api.scale_to_zero_inference_endpoint(
            name=name, namespace=namespace, token=token
        )
    except HfHubHTTPError as error:
