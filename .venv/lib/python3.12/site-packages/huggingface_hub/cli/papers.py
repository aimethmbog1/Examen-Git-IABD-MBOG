...
def papers_ls(
    ...
    month: Annotated[
        str | None,
        typer.Option(
            help="Month to filter by in ISO format (YYYY-MM), e.g. '2025-02'."
        ),
    ] = None,
    submitter: Annotated[
        str | None,
    ...
    for paper_info in api.list_daily_papers(
        ...
        item["submitted_by_name"] = (
            submitted_by.get("fullname") or submitted_by.get("username") or ""
        )
        results.append(item)
    out.table(
        results,
...
def papers_search(
    ...
    api = get_hf_api(token=token)
    results = [
        _dataclass_to_dict(paper_info)
        for paper_info in api.list_papers(query=query, limit=limit)
    ]
...
def papers_info(
    paper_id: Annotated[
        str, typer.Argument(help="The arXiv paper ID (e.g. '2502.08025').")
    ],
    token: TokenOpt = None,
) -> None:
