...
def repo_list(
    ...
    out.table(
        items, id_key="id", alignments={"storage": "right", "%_of_total": "right"}
    )
...
def repo_duplicate(
    ...
    out.result(
        "Repo duplicated", from_id=from_id, to_id=repo_url.repo_id, url=str(repo_url)
    )
...
def repo_delete(
    ...
    out.confirm(
        f"You are about to permanently delete {repo_type.value} '{repo_id}'. Proceed?",
        yes=yes,
    )
    api = get_hf_api(token=token)
    api.delete_repo(
...
@repos_cli.command(
    "move", examples=["hf repos move old-namespace/my-model new-namespace/my-model"]
)
def repo_move(
    from_id: RepoIdArg,
    to_id: RepoIdArg,
...
def repo_settings(
    ...
    api.update_repo_settings(
        ...
        gated=(
            ...
            if gated is GatedChoices.false
            else gated.value
        ),
        visibility="private" if private else "public" if public else "protected" if protected else None,  # type: ignore [arg-type]
        repo_type=repo_type.value,
...
def branch_create(
    ...
    out.result(
        "Branch created", branch=branch, repo_type=repo_type.value, repo_id=repo_id
    )
...
def branch_delete(
    ...
    out.result(
        "Branch deleted", branch=branch, repo_type=repo_type.value, repo_id=repo_id
    )
...
def tag_list(
    ...
    try:
        ...
        raise CLIError(f"{repo_type_str.capitalize()} '{repo_id}' not found.") from e
    items = [
        {"name": t.name, "target_commit": t.target_commit, "ref": t.ref}
        for t in refs.tags
    ]
    out.table(items)
