...
_EXTRA_COLORS: tuple[Color, Color, Color] = (
    (168, 176, 188),
    (128, 136, 148),
    (90, 98, 110),
)
...
def _fill_poly(
    buf: list[list[Color | None]], verts: list[tuple[int, int]], color: Color
) -> None:
    bh = len(buf)
...
def _draw_line(
    buf: list[list[Color | None]], x0: int, y0: int, x1: int, y1: int, color: Color
) -> None:
    bh = len(buf)
    bw = len(buf[0]) if buf else 0
    dx = abs(x1 - x0)
        ...
        for col in range(last + 1):
            ...
            if nbg != cbg:
                esc += (
                    "\033[49m"
                    if nbg is None
                    else f"\033[48;2;{nbg[0]};{nbg[1]};{nbg[2]}m"
                )
                cbg = nbg
            parts.append(esc + ch)
...
def _render_base_buffer(city: CityData) -> list[list[Color | None]]:
    ...
    sorted_tiles = sorted(
        city.tiles, key=lambda t: (t.grid_row + t.grid_col, t.grid_col)
    )
...
def _build_summary(
    ...
    for rtype in order:
        ...
        lines.append(
            f"    {len(group)} repos · {format_size(storage, human_readable=True)}"
        )
        lines.append("")
    ...
    for ri, row in enumerate(_CURSOR_GRID):
        for ci, ch in enumerate(row):
            if ch in _CURSOR_PALETTE:
                pixels.append(
                    (ci - len(row) // 2, ri - _CURSOR_H + 1, _CURSOR_PALETTE[ch])
                )
    return pixels
...
def run_city_game(repos: list[RepoStorageInfo]) -> None:
    ...
    if term.columns < _MIN_TERM_W or term.lines < _MIN_TERM_H:
        ...
        print(
            f"Please resize to at least {_MIN_TERM_W}×{_MIN_TERM_H} to explore the city!"
        )
        return
...
def _game_loop(city: CityData, cur_row: int, cur_col: int) -> None:
    tile_map: dict[tuple[int, int], TileInfo] = {
        (t.grid_row, t.grid_col): t for t in city.tiles
    }
    city = dataclasses.replace(
        city, buf_h=city.buf_h + _CURSOR_PAD, y_off=city.y_off + _CURSOR_PAD
    )
...
def _highlight_tile(
    buf: list[list[Color | None]], city: CityData, tile: TileInfo
) -> None:
    cx = city.x_off + (tile.grid_col - tile.grid_row) * _DX
    cy = city.y_off + (tile.grid_col + tile.grid_row) * _DY
    h = tile.height
...
def _build_info_panel(tile: TileInfo | None, city: CityData, max_w: int) -> list[str]:
    ...
    if tile.repo is None:
        ...
        lines.append(
            f"{indent}{gray}{format_size(city.extra_storage, human_readable=True)} combined{reset}"
        )
        return lines
