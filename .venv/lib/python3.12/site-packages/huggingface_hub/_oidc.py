...
def get_oidc_token(
    *, provider: Provider | str | None = None, audience: str | None = None
) -> str:
    ...
    if provider is None:
        raise OIDCError(
            f"No supported CI OIDC provider detected. Trusted Publishers currently supports: {supported}."
        )
    if provider == Provider.GITHUB:
        return _get_github_oidc_token(audience)
    raise NotImplementedError(
        f"OIDC provider '{provider}' is not supported yet. Supported: {supported}."
    )


def exchange_oidc_token(
    *, subject_token: str, resource: str, endpoint: str | None = None
) -> dict:
...
def oidc_login(
    *,
    ...
    return exchange_oidc_token(
        subject_token=subject_token, resource=resource, endpoint=endpoint
    )
