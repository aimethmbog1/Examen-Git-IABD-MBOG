                    ...
                    if found_sep:
                        ...
                        if strict_parsing:
                            raise QuerystringParseError(
                                "Skipping duplicate ampersand at %d" % i, offset=i
                            )
                        else:
        ...
        *,
        ...
        if len(boundary) > MAX_BOUNDARY_LENGTH:
            raise FormParserError(
                f"Boundary length {len(boundary)} exceeds maximum of {MAX_BOUNDARY_LENGTH}"
            )
        self.boundary = b"\r\n--" + boundary
            ...
            if state == MultipartState.START:
                ...
                if c != CR:
                    ...
                    if current_header_count > self.max_header_count:
                        raise MultipartParseError(
                            "Maximum header count exceeded", offset=i
                        )
                    current_header_size = 0
                ...
                if index == 0:
                    ...
                    if i0 >= 0:
                        ...
                        k = data.rfind(
                            boundary[:1], max(i, length - boundary_length + 1), length
                        )
        ...
        if content_type == "application/octet-stream":
            ...
            def on_headers_finished() -> None:
                ...
                if field_name is None:
                    raise FormParserError(
                        f'Field name not found in Content-Disposition: "{content_disp!r}"'
                    )

                # Create the proper class.
                content_type_b = headers.get(b"content-type")
                content_type = (
                    content_type_b.decode("latin-1")
                    if content_type_b is not None
                    else None
                )
...
def create_form_parser(
    ...
    form_parser = FormParser(
        content_type, on_field, on_file, boundary=boundary, config=config
    )
