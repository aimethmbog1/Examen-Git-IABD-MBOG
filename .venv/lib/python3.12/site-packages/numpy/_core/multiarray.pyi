...
@overload
def where(
    condition: ArrayLike, x: None = None, y: None = None, /
) -> tuple[NDArray[intp], ...]: ...
@overload
def where(
...
def can_cast(
    from_: ArrayLike | DTypeLike, to: DTypeLike, casting: _CastingKind = "safe"
) -> bool: ...
...
def bincount(
    x: ArrayLike, /, weights: ArrayLike | None = None, minlength: SupportsIndex = 0
) -> NDArray[intp]: ...
def copyto(
    dst: ndarray,
    src: ArrayLike,
...
@overload
def packbits(
    a: _ArrayLikeInt_co, /, axis: None = None, bitorder: _BitOrder = "big"
) -> ndarray[tuple[int], dtype[uint8]]: ...
@overload
def packbits(
    a: _ArrayLikeInt_co, /, axis: SupportsIndex, bitorder: _BitOrder = "big"
) -> NDArray[uint8]: ...
@overload
def unpackbits(
    a: _ArrayLike[uint8],
...
#
def datetime_data(
    dtype: str | _DTypeLike[datetime64 | timedelta64], /
) -> tuple[str, int]: ...
