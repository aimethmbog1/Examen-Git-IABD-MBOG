...
class TestCasting:
    ...
    def test_time_to_time(
        ...
        if casting == Casting.unsafe and (
            to_dt == "m8" or to_dt == "M8"
        ):  # noqa: PLR1714
    ...
    @pytest.mark.parametrize(
        "to_dtype", np.typecodes["AllInteger"] + np.typecodes["AllFloat"]
    )
    @pytest.mark.parametrize(
        "from_dtype", np.typecodes["AllInteger"] + np.typecodes["AllFloat"]
    )
    @pytest.mark.filterwarnings("ignore::numpy.exceptions.ComplexWarning")
    ...
    @pytest.mark.parametrize("to_dtype", np.typecodes["AllInteger"])
    @pytest.mark.parametrize("from_dtype", np.typecodes["AllFloat"])
