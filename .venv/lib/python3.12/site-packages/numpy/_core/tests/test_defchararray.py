...
class TestComparisons:
    def A(self):
        return np.array([["abc", "abcc", "123"], ["789", "abc", "xyz"]]).view(
            np.char.chararray
        )

    def B(self):
        return np.array([["efg", "efg", "123  "], ["051", "efgg", "tuv"]]).view(
            np.char.chararray
        )

    def test_not_equal(self):
        A, B = self.A(), self.B()
        assert_array_equal((A != B), [[True, True, False], [True, True, True]])

    def test_equal(self):
        A, B = self.A(), self.B()
        assert_array_equal((A == B), [[False, False, True], [False, False, False]])

    def test_greater_equal(self):
        A, B = self.A(), self.B()
        assert_array_equal((A >= B), [[False, False, True], [True, False, True]])

    def test_less_equal(self):
        A, B = self.A(), self.B()
        assert_array_equal((A <= B), [[True, True, True], [False, True, False]])

    def test_greater(self):
        A, B = self.A(), self.B()
        assert_array_equal((A > B), [[False, False, False], [True, False, True]])

    def test_less(self):
        A, B = self.A(), self.B()
        assert_array_equal((A < B), [[True, True, False], [False, True, False]])
...
class TestComparisonsMixed2(TestComparisons):
    ...
    def A(self):
        return np.array([["abc", "abcc", "123"], ["789", "abc", "xyz"]], np.str_).view(
            np.char.chararray
        )
...
class TestInformation:
    ...
    def test_find(self, dtype, encode):
        ...
        C = (
            np.array(["ABCDEFGHIJKLMNOPQRSTUVWXYZ", "01234567890123456789012345"]).view(
                np.char.chararray
            )
        ).astype(dtype)
    ...
    def test_isspace(self):
        ...
        assert_array_equal(
            A.isspace(), [[False, False], [False, False], [False, False]]
        )
    ...
    def test_istitle(self):
        ...
        assert_array_equal(
            A.istitle(), [[False, False], [False, False], [False, False]]
        )
...
class TestMethods:
    ...
    def test_isnumeric(self):
        ...
        assert_array_equal(
            B.isnumeric(), [[False, False], [True, False], [False, False]]
        )
    ...
    def test_isdecimal(self):
        ...
        assert_array_equal(
            B.isdecimal(), [[False, False], [True, False], [False, False]]
        )


class TestOperations:
    def A(self):
        return np.array([["abc", "123"], ["789", "xyz"]]).view(np.char.chararray)

    def B(self):
        return np.array([["efg", "456"], ["051", "tuv"]]).view(np.char.chararray)
    ...
    @pytest.mark.parametrize(
        "data", [["plate", "   ", "shrimp"], [b"retro", b"  ", b"encabulator"]]
    )
