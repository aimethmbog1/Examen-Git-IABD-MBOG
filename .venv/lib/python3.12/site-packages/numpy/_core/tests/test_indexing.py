...
class TestFancyIndexingCast:
    def test_boolean_index_cast_assign(self):
        ...
        pytest.warns(ComplexWarning, zero_array.__setitem__, bool_index, np.array([1j]))
...
class TestMultiIndexingAutomated:
    ...
    def _get_multi_index(self, arr, indices):
        ...
        for ax, indx in enumerate(in_indices):
            ...
            elif indx.ndim == 0 and not (-arr.shape[ax] <= indx < arr.shape[ax]):
                raise IndexError
            if indx.ndim == 0:
    ...
    def test_boolean(self):
        ...
        self._check_multi_index(a, (np.zeros_like(a, dtype=bool),))
        self._check_multi_index(a, (np.zeros_like(a, dtype=bool)[..., 0],))
        self._check_multi_index(a, (np.zeros_like(a, dtype=bool)[None, ...],))
    ...
    def test_multidim(self):
        ...
        with warnings.catch_warnings():
            ...
            for simple_pos in [0, 2, 3]:
                tocheck = [fill_indices, complex_indices, fill_indices, fill_indices]
...
class TestFlatiterIndexing:
    ...
    def test_flatiter_indexing_boolean(self):
        ...
        with pytest.raises(
            IndexError, match="boolean index did not match indexed flat iterator"
        ):
            a.flat[wrong_mask]
    ...
    def test_flatiter_indexing_fancy(self):
        ...
        with pytest.raises(
            IndexError, match="boolean indices for iterators are not supported"
        ):
            a.flat[[True, True]]
    ...
    def test_flatiter_indexing_slice_assign(self):
        ...
        a = np.arange(9).reshape((3, 3))
        with pytest.raises(
            IndexError, match="Assigning to a flat iterator with a 0-D index"
        ):
            a.flat[()] = 70
