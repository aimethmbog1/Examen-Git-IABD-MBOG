...
class Test_X86_Features(AbstractTest):
    ...
    features_groups["AVX512_SPR"] = features_groups["AVX512_ICL"] + [
        "AVX512FP16",
        "AVX512BF16",
    ]
...
class Test_ARM_Features(AbstractTest):
    ...
    def load_flags(self):
        ...
        is_rootfs_v8 = not re.match(r"^armv[0-9]+l$", machine) and (
            int("0" + next(iter(arch))) > 7 if arch else 0
        )
        if re.match(r"^(aarch64|AARCH64)", machine) or is_rootfs_v8:
            self.features_map = {"NEON": "ASIMD", "HALF": "ASIMD", "VFPV4": "ASIMD"}
        else:
            self.features_map = {
