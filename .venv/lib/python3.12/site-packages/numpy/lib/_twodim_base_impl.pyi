...
# to `Never`.
_ArrayNoD: TypeAlias = np.ndarray[
    tuple[Never] | tuple[Never, Never], np.dtype[_ScalarT]
]
...
_MaskFunc: TypeAlias = Callable[
    [NDArray[np.int_], _T],
    NDArray[_Number_co | np.timedelta64 | np.datetime64 | np.object_],
]

_Indices2D: TypeAlias = tuple[_Array1D[np.intp], _Array1D[np.intp]]
_Histogram2D: TypeAlias = tuple[
    _Array2D[np.float64], _Array1D[_ScalarT], _Array1D[_ScalarT]
]
...
@overload
def diag(
    v: _ArrayNoD[_ScalarT] | Sequence[Sequence[_ScalarT]], k: int = 0
) -> NDArray[_ScalarT]: ...
@overload
def diag(
    v: _Array2D[_ScalarT] | Sequence[Sequence[_ScalarT]], k: int = 0
) -> _Array1D[_ScalarT]: ...
@overload
def diag(
    v: _Array1D[_ScalarT] | Sequence[_ScalarT], k: int = 0
) -> _Array2D[_ScalarT]: ...
@overload
...
@overload
def vander(
    x: _ArrayLike1D[_NumberObjectT], N: int | None = None, increasing: bool = False
) -> _Array2D[_NumberObjectT]: ...
@overload
def vander(
    x: _ArrayLike1D[np.bool] | list[int], N: int | None = None, increasing: bool = False
) -> _Array2D[np.int_]: ...
@overload
def vander(
    x: list[float], N: int | None = None, increasing: bool = False
) -> _Array2D[np.float64]: ...
@overload
def vander(
    x: list[complex], N: int | None = None, increasing: bool = False
) -> _Array2D[np.complex128]: ...
@overload  # fallback
def vander(
    x: Sequence[_NumberLike_co], N: int | None = None, increasing: bool = False
) -> _Array2D[Any]: ...
