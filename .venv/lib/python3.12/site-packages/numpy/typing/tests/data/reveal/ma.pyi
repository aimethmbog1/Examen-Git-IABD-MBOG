...
assert_type(
    MAR_1d.argpartition(1, axis=0, kind="introselect", order="K"), MaskedArray[np.intp]
)
...
assert_type(
    MAR_f4.compress([True, False]), np.ma.MaskedArray[tuple[int], np.dtype[np.float32]]
)
assert_type(MAR_f4.compress([True, False], axis=0), MaskedArray[np.float32])
assert_type(
    MAR_f4.compress([True, False], axis=0, out=MAR_subclass), MaskedArraySubclassC
)
...
assert_type(
    MAR_2d_f4.repeat(MAR_i8), np.ma.MaskedArray[tuple[int], np.dtype[np.float32]]
)
assert_type(
    MAR_2d_f4.repeat(MAR_i8, axis=None),
    np.ma.MaskedArray[tuple[int], np.dtype[np.float32]],
)
...
assert_type(np.ma.allclose(AR_f4, MAR_f4, rtol=0.4, atol=0.3), bool)
...
assert_type(
    np.ma.getmask(MAR_2d_f4),
    np.ndarray[tuple[int, int], np.dtype[np.bool]] | _NoMaskType,
)
...
assert_type(
    MAR_2d_f4.swapaxes(1, 0), np.ma.MaskedArray[tuple[int, int], np.dtype[np.float32]]
)
...
assert_type(
    MAR_2d_V[["field_0", "field_1"]],
    np.ma.MaskedArray[tuple[int, int], np.dtype[np.void]],
)
...
assert_type(
    MAR_2d_f4.harden_mask(), np.ma.MaskedArray[tuple[int, int], np.dtype[np.float32]]
)
assert_type(MAR_i8.harden_mask(), MaskedArray[np.int64])
assert_type(
    MAR_2d_f4.soften_mask(), np.ma.MaskedArray[tuple[int, int], np.dtype[np.float32]]
)
...
assert_type(
    MAR_2d_f4.recordmask,
    np.ma.MaskType | np.ndarray[tuple[int, int], np.dtype[np.ma.MaskType]],
)

assert_type(MAR_2d_f4.anom(), np.ma.MaskedArray[tuple[int, int], np.dtype[np.float32]])
assert_type(
    MAR_2d_f4.anom(axis=0, dtype=np.float16),
...
assert_type(
    MAR_2d_f4.transpose(), np.ma.MaskedArray[tuple[int, int], np.dtype[np.float32]]
)
assert_type(
    MAR_2d_f4.transpose(1, 0), np.ma.MaskedArray[tuple[int, int], np.dtype[np.float32]]
)
assert_type(
    MAR_2d_f4.transpose((1, 0)),
    np.ma.MaskedArray[tuple[int, int], np.dtype[np.float32]],
)
...
assert_type(
    MAR_c8.reshape(2, 3, 4, 5),
    np.ma.MaskedArray[tuple[int, int, int, int], np.dtype[np.complex64]],
)
assert_type(
    MAR_td64.reshape(()), np.ma.MaskedArray[tuple[()], np.dtype[np.timedelta64]]
)
assert_type(MAR_s.reshape([]), np.ma.MaskedArray[tuple[()], np.dtype[np.str_]])
assert_type(
    MAR_V.reshape((480, 720, 4)),
    np.ma.MaskedArray[tuple[int, int, int], np.dtype[np.void]],
)
...
assert_type(
    MAR_2d_f4.view(dtype=np.float16),
    np.ma.MaskedArray[tuple[int, int], np.dtype[np.float16]],
)
assert_type(
    MAR_2d_f4.view(dtype=np.dtype(np.float16)),
    np.ma.MaskedArray[tuple[int, int], np.dtype[np.float16]],
)
...
assert_type(
    MAR_f8.argsort(endwith=True, fill_value=1.5, stable=False), MaskedArray[np.intp]
)

assert_type(
    MAR_2d_f4.flat, np.ma.core.MaskedIterator[tuple[int, int], np.dtype[np.float32]]
)
...
assert_type(
    np.ma.MaskedArray(np.array([1, 2, 3]), [True, True, False], np.float16),
    MaskedArray[np.float16],
)
assert_type(
    np.ma.MaskedArray(np.array([1, 2, 3]), dtype=np.float16), MaskedArray[np.float16]
)
