...
assert_type(
    np.testing.build_err_msg(np.arange(9).reshape(3, 3), "test", verbose=False), str
)
...
assert_type(
    np.testing.assert_array_compare(func2, AR_i8, AR_f8, precision=np.int64()), None
)
...
assert_type(
    np.testing.assert_raises_regex(RuntimeWarning, re.compile(b"test"), func3, 5), None
)
...
assert_type(
    np.testing.assert_array_max_ulp(AR_i8, AR_f8, dtype=np.float32), npt.NDArray[Any]
)
...
assert_type(
    np.testing.tempdir(prefix=b"test"), contextlib._GeneratorContextManager[bytes]
)
assert_type(
...
assert_type(
    np.testing.temppath("test_dir", text=True), contextlib._GeneratorContextManager[str]
)
assert_type(
    np.testing.temppath(prefix=b"test"), contextlib._GeneratorContextManager[bytes]
)
assert_type(
