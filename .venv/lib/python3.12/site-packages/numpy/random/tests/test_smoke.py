...
class RNG:
    ...
    def test_standard_exponential_float_log(self):
        rg = self._create_rng().rg
        randoms = rg.standard_exponential(10, dtype="float32", method="inv")
        assert_(len(randoms) == 10)
        assert randoms.dtype == np.float32
        params_0(partial(rg.standard_exponential, dtype="float32", method="inv"))
    ...
    def test_binomial(self):
        rg = self._create_rng().rg
        assert_(rg.binomial(10, 0.5) >= 0)
        assert_(rg.binomial(1000, 0.5) >= 0)
    ...
    def test_pickle(self):
        ...
        assert_(comp_state(rg.bit_generator.state, unpick.bit_generator.state))
    ...
    def test_seed_array(self):
        ...
        seed = 2 ** np.mod(np.arange(1500, dtype=dtype), data.seed_vector_bits - 1) + 1
        bg = data.bit_generator(seed)
        state1 = bg.state
        bg = data.bit_generator(seed[0])
        state2 = bg.state
