...
class GraalPy(ViaGlobalRefVirtualenvBuiltin, ABC):
    ...
    def can_describe(cls, interpreter: PythonInfo) -> bool:
        return interpreter.implementation == "GraalPy" and super().can_describe(
            interpreter
        )
    ...
    def _executables(
        ...
        tuple[Path, list[str], RefMust, RefWhen], None, None
    ...
    def sources(
        cls, interpreter: PythonInfo
    ) -> Generator[PathRefToDest]:  # ty: ignore[invalid-method-override]
        yield from super().sources(interpreter)
        python_dir = (
            Path(interpreter.system_executable).resolve().parent
...
class GraalPyWindows(GraalPy, WindowsSupports):
    ...
    def set_pyenv_cfg(self) -> None:
        ...
        ] = self.interpreter.system_executable  # ty: ignore[invalid-assignment]
