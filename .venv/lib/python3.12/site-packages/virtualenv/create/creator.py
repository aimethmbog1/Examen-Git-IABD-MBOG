...
class Creator(ABC):
    ...
    @classmethod
    def can_create(
        cls, interpreter: PythonInfo
    ) -> CreatorMeta | bool | None:  # noqa: ARG003
    ...
    def set_pyenv_cfg(self) -> None:
        ...
        system_executable = (
            self.interpreter.system_executable or self.interpreter.executable
        )
        ...
        self.pyenv_cfg["version"] = ".".join(
            str(i) for i in self.interpreter.version_info[:3]
        )
        ...
        if self.prompt is not None:
            prompt_value = (
                os.path.basename(os.getcwd()) if self.prompt == "." else self.prompt
            )
...
def get_env_debug_info(
    env_exe: Path, debug_script: Path, app_data: AppData, env: dict[str, str]
) -> dict[str, Any]:
    env = env.copy()
